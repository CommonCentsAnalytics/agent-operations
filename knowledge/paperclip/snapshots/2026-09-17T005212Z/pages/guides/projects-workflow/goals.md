> Official reference snapshot. Source: https://docs.paperclip.ing/guides/projects-workflow/goals/
> Retrieved: 2026-09-17T00:52:14.848470+00:00
...
[Image reference: Goals list](https://docs.paperclip.ing/guides/projects-workflow/goals/user-guides/screenshots/dark/goals/list.png)
...
[Image reference: Goal detail](https://docs.paperclip.ing/guides/projects-workflow/goals/user-guides/screenshots/dark/goals/detail.png)
...
- **Properties toggle** — a small slider icon on the right that re-opens the properties panel if you closed it. The properties panel is where you edit level, status, and other metadata without having to leave the page. The parent goal is shown here but is not editable from the panel — see [Reparenting and flattening](https://docs.paperclip.ing/guides/projects-workflow/goals/#reparenting-and-flattening).
...
- **From the project side** — open the project's Configuration tab (see [Projects](https://docs.paperclip.ing/guides/projects-workflow/projects/)) and add the goal under the Goals field. The change saves automatically.
...
The same change is available on the API via `PATCH /goals/{id}` with a `parentId` field (pass `null` to promote the goal to the root). The tree re-renders once the change lands. See the [Goal CLI reference](https://docs.paperclip.ing/reference/cli/goal/#update-a-goal) for the full flag list.
...
- The project layer that sits under goals: [Projects](https://docs.paperclip.ing/guides/projects-workflow/projects/).
- How issues pick up agents: [Managing Tasks](https://docs.paperclip.ing/guides/day-to-day/issues/).
- How the CEO builds a strategy that references your goals: [Approvals](https://docs.paperclip.ing/guides/day-to-day/approvals/).

> Official reference snapshot. Source: https://docs.paperclip.ing/guides/getting-started/installation/
> Retrieved: 2026-09-17T00:52:13.205719+00:00
...
See [Service](https://docs.paperclip.ing/reference/cli/service/) for the full set of commands, and the [advanced deployment docs](https://docs.paperclip.ing/reference/deploy/overview/) for server-side deployment patterns.
...
- **Hosted PostgreSQL** — set `DATABASE_URL=postgres://...` in `~/paperclip.env` before onboarding. Use the pooled connection (port 6543 on Supabase) for the app and the direct connection for migrations. See [Database deployment](https://docs.paperclip.ing/reference/deploy/database/).
- **Object storage** — set `PAPERCLIP_STORAGE_MODE=s3` plus the relevant S3 env vars. See [Storage deployment](https://docs.paperclip.ing/reference/deploy/storage/).
- **Private team server over Tailscale** instead of a public domain — skip Nginx/Certbot and use `PAPERCLIP_DEPLOYMENT_EXPOSURE=private` with `PAPERCLIP_BIND=tailnet`. See [Tailscale private access](https://docs.paperclip.ing/reference/deploy/tailscale-private-access/). On a private, authenticated instance you can also skip the CLI invite for the first user: the very first person to sign in from a browser can claim themselves as the instance admin straight from the UI — whoever clicks first wins, and everyone else is locked out of the claim. See [Claim first instance admin](https://docs.paperclip.ing/reference/api/instance-admin/#claim-first-instance-admin) for the details.
- **Docker instead of bare metal** — a production-ready image and Compose file ship in the repo. See [Docker deployment](https://docs.paperclip.ing/reference/deploy/docker/).
...
> **Note:** There is also an **unofficial, community-maintained desktop app** for macOS that wraps Paperclip in a regular Mac application. It is not built or supported by the Paperclip team. If you're curious, see [Community Desktop App](https://docs.paperclip.ing/how-to/community-desktop-app/).

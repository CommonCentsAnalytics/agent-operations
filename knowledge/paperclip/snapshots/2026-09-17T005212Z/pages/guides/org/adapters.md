> Official reference snapshot. Source: https://docs.paperclip.ing/guides/org/adapters/
> Retrieved: 2026-09-17T00:52:14.623087+00:00
...
This guide walks through the page itself. For the per-adapter config fields (working directory, model names, timeouts, environment variables) see [Agent Adapters](https://docs.paperclip.ing/guides/org/agent-adapters/). For the technical catalog of every adapter type — wire format, capabilities, model listings — see the [Adapter Reference](https://docs.paperclip.ing/reference/adapters/overview/).
...
[Image reference: Adapter Manager list](https://docs.paperclip.ing/guides/org/adapters/user-guides/screenshots/dark/adapters/list.png)
...
> **Tip:** The adapter package must export a `createServerAdapter()` function that conforms to Paperclip's adapter contract. See [Creating an Adapter](https://docs.paperclip.ing/reference/adapters/creating-an-adapter/).
...
**Reload** (circular-arrow icon) Re-imports the adapter module in place. Use this after changing the code of a local-path adapter — the runtime hot-swaps the module and invalidates any cached config parsers or UI schemas. Existing agents pick up the change on their next run.

**Reinstall** (download icon, npm installs only) Pulls the latest version of the package from npm and reloads it. Clicking the icon opens a confirmation dialog that shows the package name, the installed version, and the latest version available on the npm registry:
...
[Image reference: Adapter configuration](https://docs.paperclip.ing/guides/org/adapters/user-guides/screenshots/dark/adapters/detail.png)

For field-by-field walkthroughs of the common built-ins, see [Agent Adapters](https://docs.paperclip.ing/guides/org/agent-adapters/). For the technical contract that adapters implement (including the UI parser schema), see [Adapter UI Parser](https://docs.paperclip.ing/reference/adapters/adapter-ui-parser/).
...
[Image reference: Health status](https://docs.paperclip.ing/guides/org/adapters/user-guides/screenshots/dark/adapters/health.png)
...
- [Agent Adapters](https://docs.paperclip.ing/guides/org/agent-adapters/) — what each adapter does and which configuration fields it exposes on the agent form.
- [Adapter Reference](https://docs.paperclip.ing/reference/adapters/overview/) — technical catalog of every built-in adapter type.
- [Creating an Adapter](https://docs.paperclip.ing/reference/adapters/creating-an-adapter/) — how to author and publish an external adapter package.
- [Adapter UI Parser](https://docs.paperclip.ing/reference/adapters/adapter-ui-parser/) — the schema contract your adapter uses to render config fields in the agent form.

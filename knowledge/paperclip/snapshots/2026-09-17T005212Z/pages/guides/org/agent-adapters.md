> Official reference snapshot. Source: https://docs.paperclip.ing/guides/org/agent-adapters/
> Retrieved: 2026-09-17T00:52:14.576901+00:00
...
[Image reference: Adapter type dropdown showing all available options](https://docs.paperclip.ing/guides/org/agent-adapters/user-guides/screenshots/dark/agents/adapter-type-dropdown.png)
...
**Working directory** The folder on your Mac where the agent does its work — reads files, writes outputs, runs commands. If you're using this agent for a software project, point this to your project folder. If you're not sure, create a new folder called `paperclip-workspace` on your Desktop and use that.
...
**Model** Which Claude model to use. The main choices:
...
**Environment variables** The agent form includes an **Environment variables** section. Add `ANTHROPIC_API_KEY` there, either as a plain value or as a secret reference. If you're not sure what key name to use, `ANTHROPIC_API_KEY` is the standard one.

**Timeout (seconds)** How long a single heartbeat run is allowed to take before Paperclip cuts it off. 300 seconds (5 minutes) is a safe default for most tasks. Complex coding tasks may need longer — you can increase it to 600 or more. Setting it too low will cause agents to time out mid-task.

[Image reference: Claude local adapter configuration form with all fields filled in](https://docs.paperclip.ing/guides/org/agent-adapters/user-guides/screenshots/dark/agents/claude-local-config-filled.png)
...
[Image reference: Codex local adapter configuration form](https://docs.paperclip.ing/guides/org/agent-adapters/user-guides/screenshots/dark/agents/codex-local-config.png)
...
- `openclaw_gateway` — remote OpenClaw instances reached over the WebSocket gateway protocol. Today, the normal path is the OpenClaw invite-prompt flow on the Company Settings page. See [OpenClaw Gateway](https://docs.paperclip.ing/reference/adapters/openclaw-gateway/).
- `http` — webhook-style invocation into your own service. Aimed at developers building custom agent integrations. See [HTTP](https://docs.paperclip.ing/reference/adapters/http/).
- `process` — runs an arbitrary local command or script. See [Process](https://docs.paperclip.ing/reference/adapters/process/).
...
**Endpoint URL** The full URL Paperclip will POST to when a heartbeat fires. This must be publicly accessible or reachable from your Paperclip instance.
...
[Image reference: HTTP adapter configuration form](https://docs.paperclip.ing/guides/org/agent-adapters/user-guides/screenshots/dark/agents/http-adapter-config.png)
...
**Command** The executable command and any arguments. The command must be accessible from the machine running Paperclip.

**Working directory** Where to run the command.

**Environment variables** Additional environment variables to set for the process (e.g., API keys, paths).

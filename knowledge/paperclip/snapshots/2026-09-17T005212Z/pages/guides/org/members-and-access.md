> Official reference snapshot. Source: https://docs.paperclip.ing/guides/org/members-and-access/
> Retrieved: 2026-09-17T00:52:14.545780+00:00
...
If you want the button-by-button pages, they're in [Company Administration](https://docs.paperclip.ing/administration/company/). If you want the exact permission table, that's [Roles & Permissions](https://docs.paperclip.ing/administration/roles-and-permissions/). This page is the "why it's shaped this way" in between.
...
[Image reference: The Members page lists human and agent principals together](https://docs.paperclip.ing/guides/org/members-and-access/user-guides/screenshots/dark/company/access.png)
...
The two combine additively: role bundle **plus** any explicit grants. Explicit grants are stored on the member independently, so they outlive role changes — promote-then-demote someone and the boxes you checked by hand are still checked. The precise bundles and the full key list live in [Roles & Permissions](https://docs.paperclip.ing/administration/roles-and-permissions/); the thing to internalize here is just *bundle + extras, extras persist*.
...
A human "Owner" is not the CEO agent's boss in the org-chart sense; they're the person who administers the company around the agents. The two models are deliberately independent. For the agent side, see [Org Structure](https://docs.paperclip.ing/guides/org/org-structure/).
...
This is the role you use when you run Paperclip as a shared platform (say, a VPS hosting several companies) rather than a personal tool. The very first instance admin is established once, through the [board-claim](https://docs.paperclip.ing/administration/cli-auth/) flow, when you move an instance into authenticated mode. After that, instance admins hand out access from the Instance Access surface. It's independent of company roles: someone can be an instance admin with no company membership at all.
...
It's a useful lens when you want to see what a teammate has actually been doing, or attribute cost to a person. You can pull the same data from the CLI with `paperclipai profile company-user <user-slug> --company-id <company-id>` (see the [CLI reference](https://docs.paperclip.ing/reference/cli/access/#profile)).
...
[Image reference: The profile settings page, where you edit your own name and avatar](https://docs.paperclip.ing/guides/org/members-and-access/user-guides/screenshots/dark/settings/profile.png)
...
These are **navigation, not authorization.** By default you're "joined" to things, and *leaving* a project or agent simply tidies it out of your personal sidebar — it doesn't revoke anyone's access, and it doesn't change what you're allowed to do. Rejoin any time and it reappears. Don't reach for resource memberships to control who can see what; that's what roles and grants are for. The mechanics are documented in the [Resource Memberships API](https://docs.paperclip.ing/reference/api/resource-memberships/).

---

## Where to go next

- [Add a human teammate](https://docs.paperclip.ing/how-to/add-a-human-teammate/) — the practical onboarding flow.
- [Roles & Permissions](https://docs.paperclip.ing/administration/roles-and-permissions/) — the exact role bundles and permission keys.
- [Company Administration](https://docs.paperclip.ing/administration/company/) — the Members page (Access and its Invites tab) and Join Requests.
- [Org Structure](https://docs.paperclip.ing/guides/org/org-structure/) — the *agent* reporting tree, the other half of "who reports to whom."

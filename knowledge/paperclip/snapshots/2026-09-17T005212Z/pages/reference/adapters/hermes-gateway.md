> Official reference snapshot. Source: https://docs.paperclip.ing/reference/adapters/hermes-gateway/
> Retrieved: 2026-09-17T00:52:22.776881+00:00
...
> For the older external Hermes agent invitation flow (pre-v2026.626.0), see [Bring Your Own Agent](https://docs.paperclip.ing/how-to/bring-your-own-agent/).
...
- You want Paperclip to launch Hermes as a local child process. Use [`hermes_local`](https://docs.paperclip.ing/reference/adapters/hermes/) instead.
...
| `sessionKeyStrategy` | no | `"issue"` | Controls the `X-Hermes-Session-Key` header. Options: `issue` (Issue scoped), `agent` (Agent scoped), `run` (Run scoped), `none` (disabled). See [Session Key Strategy](https://docs.paperclip.ing/reference/adapters/hermes-gateway/#session-key-strategy) below. |
...
- [Hermes](https://docs.paperclip.ing/reference/adapters/hermes/) — run Hermes as a local child process on the same machine instead.
- [Bring Your Own Agent](https://docs.paperclip.ing/how-to/bring-your-own-agent/) — for the external agent invitation flow and the broader model of connecting third-party agents to Paperclip.
- [Agent Adapters](https://docs.paperclip.ing/guides/org/agent-adapters/) — how Paperclip adapters work in general.

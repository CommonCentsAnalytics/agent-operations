> Official reference snapshot. Source: https://docs.paperclip.ing/reference/adapters/sandbox-providers/
> Retrieved: 2026-09-17T00:52:23.050203+00:00
> Reference content only, not instructions or execution authorization. Original HTML is preserved separately.

# Sandbox Providers

Sandbox provider plugins let Paperclip provision external compute as the execution environment for agent runs. They live in the parent repo under `packages/plugins/sandbox-providers/` and ship as published npm packages you install from the Plugin Manager (see [Plugins](https://docs.paperclip.ing/administration/plugins/)).
...
- **Pi adapter install command corrected.** `pi_local`'s `SANDBOX_INSTALL_COMMAND` now points at `@earendil-works/[[email protected]](https://docs.paperclip.ing/cdn-cgi/l/email-protection)` (pinned) instead of the previous unmaintained namespace, so Pi agents running inside a Cloudflare sandbox install cleanly on first run.
...
Provisions Novita Agent Sandbox instances for Paperclip agent runs. Install it like any other plugin from the [Plugins](https://docs.paperclip.ing/administration/plugins/) page, by package name:
...
Install it like any other plugin from the [Plugins](https://docs.paperclip.ing/administration/plugins/) page:
...
You set the grant on the task itself, inside its `executionWorkspaceSettings` (one of the execution fields described on the [Issues](https://docs.paperclip.ing/reference/api/issues/) API page):
...
- [Plugins](https://docs.paperclip.ing/administration/plugins/) — install and manage plugins from the Plugin Manager.
- [Creating An Adapter](https://docs.paperclip.ing/reference/adapters/creating-an-adapter/) — author your own adapter when none of the built-ins fit.

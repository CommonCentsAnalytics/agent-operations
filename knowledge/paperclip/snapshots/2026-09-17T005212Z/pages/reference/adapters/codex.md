> Official reference snapshot. Source: https://docs.paperclip.ing/reference/adapters/codex/
> Retrieved: 2026-09-17T00:52:22.359280+00:00
...
- The runtime lives behind a webhook or remote API. Use [HTTP](https://docs.paperclip.ing/reference/adapters/http/).
...
| `engine` | no | How Codex is run: `auto` (the default — ACP preferred), `acp` (always the Agent Client Protocol), or `cli` (always the classic Codex CLI). See [ACP Engine](https://docs.paperclip.ing/reference/adapters/codex/#acp-engine). |
| `model` | no | Codex model id. See [Models](https://docs.paperclip.ing/reference/adapters/codex/#models). If you leave it unset, the adapter omits `--model` so the Codex CLI uses its own default. |

> Official reference snapshot. Source: https://docs.paperclip.ing/reference/adapters/kimi-local/
> Retrieved: 2026-09-17T00:52:22.804570+00:00
...
- The runtime lives behind a webhook or remote API. Use [HTTP](https://docs.paperclip.ing/reference/adapters/http/) or [OpenClaw Gateway](https://docs.paperclip.ing/reference/adapters/openclaw-gateway/).
- You only need a shell command or script. Use [Process](https://docs.paperclip.ing/reference/adapters/process/).
...
| `engine` | no | Which lane runs Kimi: the adapter defaults to the ACP engine (`kimi acp`) and falls back to the headless CLI when ACP prerequisites are unavailable. Set `acp` or `cli` to require a specific lane. See [ACP Engine](https://docs.paperclip.ing/reference/adapters/kimi-local/#acp-engine). |
| `model` | no | Kimi model alias (`provider/model`). Defaults to `kimi-code/kimi-for-coding`. See [Models](https://docs.paperclip.ing/reference/adapters/kimi-local/#models). |
| `effort` | no | Thinking effort (`low`, `medium`, `high`, `max`). CLI lane only. See [Thinking Effort](https://docs.paperclip.ing/reference/adapters/kimi-local/#thinking-effort). |

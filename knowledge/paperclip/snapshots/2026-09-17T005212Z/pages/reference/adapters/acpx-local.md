> Official reference snapshot. Source: https://docs.paperclip.ing/reference/adapters/acpx-local/
> Retrieved: 2026-09-17T00:52:22.605201+00:00
...
>  
> 
> [Claude Code](https://docs.paperclip.ing/reference/adapters/claude-code/) · [Codex](https://docs.paperclip.ing/reference/adapters/codex/) · [Gemini CLI](https://docs.paperclip.ing/reference/adapters/gemini-cli/)
...
- **Environment variables travel through ACPX session options.** Instead of being baked into a shell script, your agent's environment (including the values Paperclip injects for a run) is handed to the ACPX runtime as part of the session options and applied when the process spawns.
...
- **Symlinks fall back to copies.** Paperclip stages runtime skills and config files into place with symlinks. On Windows, creating a symlink can fail with an `EPERM` permission error; when that happens, the engine copies the file instead so setup still succeeds.
...
- [Claude Code](https://docs.paperclip.ing/reference/adapters/claude-code/) — Claude harness, ACP by default.
- [Codex](https://docs.paperclip.ing/reference/adapters/codex/) — Codex harness, ACP by default.
- [Gemini CLI](https://docs.paperclip.ing/reference/adapters/gemini-cli/) — Gemini harness, ACP by default.
- [Adapters Overview](https://docs.paperclip.ing/reference/adapters/overview/) — the full list of adapters you can pick today.
- [Creating An Adapter](https://docs.paperclip.ing/reference/adapters/creating-an-adapter/) — author your own when none of the built-ins fit.

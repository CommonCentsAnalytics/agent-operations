> Official reference snapshot. Source: https://docs.paperclip.ing/reference/api/agents/
> Retrieved: 2026-09-17T00:52:17.987596+00:00
...
For the full lifecycle of the secrets themselves (creating, rotating, delivery modes) see the [Secrets reference](https://docs.paperclip.ing/reference/api/secrets/). Operators decide which secrets each agent can reach from the **Secret access** editor in agent settings; see [Secret folders](https://docs.paperclip.ing/administration/secret-folders/) for that picker.
...
- `delivery` — `env`, `api`, or `both` (see [Delivery modes](https://docs.paperclip.ing/reference/api/secrets/#delivery-modes-env-vs-access))
- `projectionClass`
...
Use the JSON route when you want to build your own visualizations. Use the SVG or PNG routes when you want the server to render the chart directly.
...
- Skills must already be installed at the company level. The full company-skill lifecycle (file shape, import, scoping, versioning) is documented in the [Skills reference](https://docs.paperclip.ing/reference/skills/).

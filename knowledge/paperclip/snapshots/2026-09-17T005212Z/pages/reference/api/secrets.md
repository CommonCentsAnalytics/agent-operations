> Official reference snapshot. Source: https://docs.paperclip.ing/reference/api/secrets/
> Retrieved: 2026-09-17T00:52:18.858839+00:00
...
See [Rotate Secret](https://docs.paperclip.ing/reference/api/secrets/#rotate-secret) for what happens when you use that last flag.
...
An `external_reference` secret used to be link-only. If the provider reports `supportsExternalValueWrites`, you can now also write a new value straight through to the external vault — see [Rotate Secret](https://docs.paperclip.ing/reference/api/secrets/#rotate-secret).
...
Returns a trimmed list of the company's secrets — just the `id`, `name`, `key`, and `status` of each. Unlike [List Secrets](https://docs.paperclip.ing/reference/api/secrets/#list-secrets), which is board-only and returns full metadata, the catalog is meant for picking a secret to reference, so it's callable by the board *or* by an agent with access to the company. The secret values themselves are never returned.
...
This endpoint does not change the secret value. There is no `value` field here, and that has not changed — every value write, including a write through to an external vault, goes through [Rotate Secret](https://docs.paperclip.ing/reference/api/secrets/#rotate-secret).
...
If you would rather do this from the board, [Connect an AWS Secrets Manager vault](https://docs.paperclip.ing/how-to/connect-aws-secrets-vault/#change-the-value-of-a-linked-aws-secret) walks through the same two choices in the UI.
...
Access-mode grants are the ones marked as API-only. Operators manage which secrets each agent can reach — and whether a grant is delivered as an environment variable or as run-bound access — from the **Secret access** editor in agent settings. The deep folder and picker UI for that editor is documented in [Secret folders](https://docs.paperclip.ing/administration/secret-folders/).
...
Full request and response examples for both routes live on the [Agents reference](https://docs.paperclip.ing/reference/api/agents/#fetch-granted-secrets) page, since they are agent-run-scoped.
...
A binding can depend on a secret proposal that hasn't been approved yet, so an agent can raise both in one go: propose the secret, then propose a binding that references the secret proposal by ID. The board can approve them together (see [Approve a proposal](https://docs.paperclip.ing/reference/api/secrets/#approve-a-proposal)).

### Who can propose

The agent-side routes require a **verified run-bound agent token** — the same live-heartbeat requirement as [Run-Bound Agent Secret Access](https://docs.paperclip.ing/reference/api/secrets/#run-bound-agent-secret-access), plus a real signed agent JWT. Task-bridge and skill-test tokens are refused, and the run is checked against the `secrets:propose` authorization action.
...
On success you get `201 Created` with the proposal record (see [Proposal fields](https://docs.paperclip.ing/reference/api/secrets/#proposal-fields)). The proposed value is never echoed back — the agent-facing view strips the value fingerprint and length as well.
...
[Image reference: The Proposals tab in company secrets settings: one pending proposal showing its path-style name, the proposing agent, the value&#39;s fingerprint and byte length, its expiry, the agent&#39;s stated reason, and Approve and Reject buttons](https://docs.paperclip.ing/reference/api/secrets/user-guides/screenshots/dark/secrets/proposals-tab.png)
...
See [Routine env map](https://docs.paperclip.ing/how-to/create-a-daily-routine/#4-optional-give-the-routine-an-env-map) for the routine-side example.

> Official reference snapshot. Source: https://docs.paperclip.ing/reference/api/instance-admin/
> Retrieved: 2026-09-17T00:52:19.342962+00:00
...
The experimental-settings response and patch body include `enableBuiltInAgents` as a boolean. Set it to `true` before using the [Built-in Agents API](https://docs.paperclip.ing/reference/api/built-in-agents/); while it is off, those routes return `404 Not Found`.
...
- [Reference → Adapters API](https://docs.paperclip.ing/reference/api/adapters/) — adapter registry endpoints.
- [Reference → Plugins API](https://docs.paperclip.ing/reference/api/plugins/) — plugin install and lifecycle.
- [Reference → Deployment → Environment Variables](https://docs.paperclip.ing/reference/deploy/environment-variables/) — the env-vars these admin surfaces interact with.

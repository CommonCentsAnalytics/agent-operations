> Official reference snapshot. Source: https://docs.paperclip.ing/reference/api/built-in-agents/
> Retrieved: 2026-09-17T00:52:18.025849+00:00
...
If you have not read it yet, start with the [API Overview](https://docs.paperclip.ing/reference/api/overview/) for the base URL, authentication, and company-scoping rules. Everything here builds on those conventions.
...
The first adapter type in each list is the default the server picks if you provision without naming one. The Reflection Coach is a little different from the other two — it ships as a **bundle** and starts life paused. More on that [below](https://docs.paperclip.ing/reference/api/built-in-agents/#the-reflection-coach).
...
| `needs_setup` | The agent exists, but its adapter config is incomplete — for example, no model or command yet. It will not run until you finish the setup, which you do by provisioning it again with the adapter details. See [Finishing first-time setup](https://docs.paperclip.ing/reference/api/built-in-agents/#finishing-first-time-setup). |
...
`POST /api/companies/{companyId}/built-in-agents/{key}/routines/{routineKey}/enable` `POST /api/companies/{companyId}/built-in-agents/{key}/routines/{routineKey}/disable` `POST /api/companies/{companyId}/built-in-agents/{key}/routines/{routineKey}/run`
...
These are the softer counterparts to the direct `agents:configure` and `skills:create` permissions. Because the coach holds only the "suggest" keys, its proposals always flow through review before anything takes effect. The full model — including which roles hold which keys — lives on the [Roles and Permissions](https://docs.paperclip.ing/administration/roles-and-permissions/) page.

---

## Where to Go Next

- [Agents API](https://docs.paperclip.ing/reference/api/agents/) — the full agent lifecycle, adapters, and configuration that built-ins are built on.
- [Roles and Permissions](https://docs.paperclip.ing/administration/roles-and-permissions/) — the `agents:create`, `tasks:assign`, and `*:suggest-changes` keys referenced here.
- [Skills reference](https://docs.paperclip.ing/reference/skills/) — how bundled skills like the Reflection Coach's are packaged, imported, and versioned.

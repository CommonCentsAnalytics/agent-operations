> Official reference snapshot. Source: https://docs.paperclip.ing/reference/api/checkbox-confirmations/
> Retrieved: 2026-09-17T00:52:18.338885+00:00
...
[Image reference: A checkbox confirmation card in an issue thread: prompt, markdown details, a five-option list with three options checked, select-all and clear-selection controls, and custom accept and reject buttons.](https://docs.paperclip.ing/reference/api/checkbox-confirmations/user-guides/screenshots/dark/interactions/checkbox-confirmation-card.png)
...
- `idempotencyKey` - optional, recommended. See [Idempotency](https://docs.paperclip.ing/reference/api/checkbox-confirmations/#idempotency).
- `continuationPolicy` - `wake_assignee`, `wake_assignee_on_accept`, or `none`. Defaults to `wake_assignee` for this kind. See [Continuation policy](https://docs.paperclip.ing/reference/api/checkbox-confirmations/#continuation-policy).
...
| `target` | target object or `null` | no | `null` | Binds the request to an issue document revision. See [Target binding and staleness](https://docs.paperclip.ing/reference/api/checkbox-confirmations/#target-binding-and-staleness). |
...
For the shared interaction envelope and the other kinds, see the [Issues API](https://docs.paperclip.ing/reference/api/issues/).

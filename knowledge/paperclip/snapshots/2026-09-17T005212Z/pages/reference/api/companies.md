> Official reference snapshot. Source: https://docs.paperclip.ing/reference/api/companies/
> Retrieved: 2026-09-17T00:52:17.965169+00:00
...
To add or remove a star, use the document route on the [Resource Memberships](https://docs.paperclip.ing/reference/api/resource-memberships/) page.
...
A large export doesn't have to arrive in one upload. When a package is big, the client slices it into parts and streams them one at a time through a **transfer** — a durable run the server tracks so an interrupted upload resumes from the parts it already has instead of starting over. The UI and `paperclipai company import` both use this path automatically; see the [Export & Import guide](https://docs.paperclip.ing/guides/power/export-import/) for the user-facing flow.

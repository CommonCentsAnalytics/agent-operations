> Official reference snapshot. Source: https://docs.paperclip.ing/reference/api/decisions/
> Retrieved: 2026-09-17T00:52:18.221398+00:00
...
Everything about that exchange is a record. The decision is stored, the options are signed so they cannot drift, the effects are executed one at a time with an audit entry each, and the outcome is available afterwards. Open decisions also show up in the [Attention](https://docs.paperclip.ing/reference/api/attention/) feed as a `decision` source, so you meet them in the same place as everything else that needs you.
...
Decisions are company-scoped, and company access is enforced on every request. If you are new to the API, read the [API Overview](https://docs.paperclip.ing/reference/api/overview/) first for base URL, authentication, and error-code conventions; this page builds on those and won't repeat them.
...
Triage feeds straight back into the attention feed: `decideBy` and `snoozedUntil` appear on each item, `sort=decide` orders by deadline, and `decideNowCount` counts what is due today. See [Attention](https://docs.paperclip.ing/reference/api/attention/) for how that ranking works.

---

## Where to go next

- [Decisions](https://docs.paperclip.ing/guides/day-to-day/decisions/) — the operator workflow these routes sit behind.
- [Attention](https://docs.paperclip.ing/reference/api/attention/) — the ranked feed where open decisions, queues, and triage all surface.
- [Decision Training](https://docs.paperclip.ing/reference/api/decision-training/) — keep a decision as a labelled training example.
- [Issues](https://docs.paperclip.ing/reference/api/issues/) — the issues decision effects act on.
- [Activity](https://docs.paperclip.ing/reference/api/activity/) — where the `decision.*`, `decision_queue.*`, and `decision_triage.*` entries land.
- [API Overview](https://docs.paperclip.ing/reference/api/overview/) — base URL, authentication, company scoping, and the shared error-code table.

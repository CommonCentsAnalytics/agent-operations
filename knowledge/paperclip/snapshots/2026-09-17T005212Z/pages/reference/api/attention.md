> Official reference snapshot. Source: https://docs.paperclip.ing/reference/api/attention/
> Retrieved: 2026-09-17T00:52:18.231956+00:00
...
If you have not read it yet, start with the [API Overview](https://docs.paperclip.ing/reference/api/overview/) for the base URL, authentication, and company-scoping rules.
...
| `queue` | Only items belonging to the [decision queue](https://docs.paperclip.ing/reference/api/decisions/) with this key. |
...
| `trainingExampleId` | Set when you have already kept this item as a [decision training](https://docs.paperclip.ing/reference/api/decision-training/) example. |
...
The full interaction lifecycle (create, respond, withdraw, expiry) lives in the [Issues API](https://docs.paperclip.ing/reference/api/issues/#interactions); this section covers the addressee and resolver-policy surface that decides how a card is routed.
...
- **Withdrawal** — the creator, the current issue assignee, or a board user can withdraw a pending card; its `result.outcome` becomes `withdrawn`. See [withdraw vs. cancel](https://docs.paperclip.ing/reference/api/issues/#withdraw-vs-cancel).
...
There is a second, different snooze. An inbox dismissal is yours alone and shows up on the item as `dismissal`. A **triage** snooze is set on the source itself, applies to everyone reading the feed, and shows up as `snoozedUntil`. Reach for triage when the whole company should stop seeing something until a date; reach for a dismissal when you personally want your own queue tidier. Triage lives on the [Decisions API](https://docs.paperclip.ing/reference/api/decisions/).

---

## Where to go next

- [Decisions](https://docs.paperclip.ing/guides/day-to-day/decisions/) — use the built-in operator workflow.
- [Decisions API](https://docs.paperclip.ing/reference/api/decisions/) — decision records, queues, and triage.
- [Approvals](https://docs.paperclip.ing/reference/api/approvals/) — act on approval records in the queue.
- [Issues](https://docs.paperclip.ing/reference/api/issues/) — work with issue-thread interactions and the issues behind many attention items.

> Official reference snapshot. Source: https://docs.paperclip.ing/reference/api/inbox-agent-policy/
> Retrieved: 2026-09-17T00:52:18.662359+00:00
...
Every route below is company-scoped, and company access is checked on every request. If you are new to the API, read the [API Overview](https://docs.paperclip.ing/reference/api/overview/) first for base URL, authentication, and error-code conventions; this page builds on those and won't repeat them.
...
> **Note.** The permission these routes check is `users:manage_permissions`, not `inbox:manage`. The `inbox:manage` permission key does exist in the permission catalog, but it governs whether an agent may *act on* an inbox — not whether it may edit the policy. See [Who the policy actually governs](https://docs.paperclip.ing/reference/api/inbox-agent-policy/#who-the-policy-actually-governs) below.
...
- [Attention](https://docs.paperclip.ing/reference/api/attention/) — the inbox surface these policies protect.
- [Agents](https://docs.paperclip.ing/reference/api/agents/) — the agents you name in `allowedAgentIds`.
- [Resource Memberships](https://docs.paperclip.ing/reference/api/resource-memberships/) — company memberships and the permission grants behind `users:manage_permissions` and `inbox:manage`.

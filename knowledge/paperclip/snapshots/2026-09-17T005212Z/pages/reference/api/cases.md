> Official reference snapshot. Source: https://docs.paperclip.ing/reference/api/cases/
> Retrieved: 2026-09-17T00:52:18.196231+00:00
...
Cases are company-scoped, and — like the rest of the control-plane API — company access is enforced on every request. If you are new to the API, read the [API Overview](https://docs.paperclip.ing/reference/api/overview/) first for base URL, authentication, and error-code conventions; this page builds on those and won't repeat them.
...
- [Issues](https://docs.paperclip.ing/reference/api/issues/) — the work objects cases link to, and the source of the issue identifiers used by `POST /api/cases/{id}/links` and `GET /api/issues/{issueId}/cases`.
- [Goals and Projects](https://docs.paperclip.ing/reference/api/goals-and-projects/) — the projects a case can belong to via `projectId`.

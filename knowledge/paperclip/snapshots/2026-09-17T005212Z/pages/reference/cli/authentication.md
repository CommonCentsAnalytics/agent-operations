> Official reference snapshot. Source: https://docs.paperclip.ing/reference/cli/authentication/
> Retrieved: 2026-09-17T00:52:19.843599+00:00
...
For the conceptual model behind personas and how they fit the operating model, see [Personas and Tokens](https://docs.paperclip.ing/administration/cli-auth/).
...
For the full list of flags shared by every client command (`--data-dir`, `--api-base`, `--api-key`, `--context`, `--profile`, `--json`), see [Common Options](https://docs.paperclip.ing/reference/cli/common-options/).
...
> **Tip:** If you want the full local setup for running Claude or Codex as an agent — a long-lived agent key plus skills installed into `~/.codex/skills` and `~/.claude/skills` plus ready-to-paste exports — use `agent local-cli <agent>` instead of minting a bare key by hand. See [Agents](https://docs.paperclip.ing/reference/cli/agent/).
...
- [Common Options](https://docs.paperclip.ing/reference/cli/common-options/) — the flags every client command shares, including API base resolution.
- [Personas and Tokens](https://docs.paperclip.ing/administration/cli-auth/) — the conceptual model behind board vs. agent identity.
- [Agents](https://docs.paperclip.ing/reference/cli/agent/) — `agent local-cli` and agent management.
- [Setup Commands](https://docs.paperclip.ing/reference/cli/setup-commands/) — bootstrapping and configuring an instance.
- [Install and Connect](https://docs.paperclip.ing/guides/getting-started/installation/) — the guided first-run walkthrough.
- [Headless Bootstrap](https://docs.paperclip.ing/reference/cli/installation/) — minting the first credential with no browser.

> Official reference snapshot. Source: https://docs.paperclip.ing/reference/cli/plugin/
> Retrieved: 2026-09-17T00:52:21.203808+00:00
...
> **Note:** Every command except `plugin init` talks to the API and accepts the common client flags. (`plugin init` runs entirely on your machine and never touches the API.) See [Common Options](https://docs.paperclip.ing/reference/cli/common-options/) for `--api-base`, `--api-key`, `--context`, `--profile`, `--data-dir`, and `--json`.
...
- [Common Options](https://docs.paperclip.ing/reference/cli/common-options/) — the client flags every API-backed plugin command accepts
- [Output and Scripting](https://docs.paperclip.ing/reference/cli/output-and-scripting/) — using `--json` to script against the CLI
- [Adapter Commands](https://docs.paperclip.ing/reference/cli/adapter/) — configuring the runtime adapters agents execute on
- [Skills Commands](https://docs.paperclip.ing/reference/cli/skills/) — the other way to extend what agents can do
- [Installation](https://docs.paperclip.ing/reference/cli/installation/) — installing the CLI and the in-repo development alias

> Official reference snapshot. Source: https://docs.paperclip.ing/reference/cli/adapter/
> Retrieved: 2026-09-17T00:52:20.514479+00:00
...
> **Note:** Most agent work in Paperclip is executed server-side by the runtime and its adapters. The CLI triggers and observes that work — see [What runs where](https://docs.paperclip.ing/guides/welcome/key-concepts/). The `adapter` commands let a board operator look under the hood of that execution layer.
...
When you create an agent you set its `adapterType`, so reading an adapter's config schema first (see below) tells you exactly which fields that agent's `payload-json` accepts. See [Agent Commands](https://docs.paperclip.ing/reference/cli/agent/) for the hire flow.
...
Every `adapter` subcommand accepts the standard client options: `--data-dir <path>`, `--api-base <url>`, `--api-key <token>`, `--context <path>`, `--profile <name>`, and `--json`. The company-scoped subcommands — `models`, `model-profiles`, `detect-model`, and `test-environment` — additionally take `-C, --company-id <id>` and require a resolvable company. See [Common Options](https://docs.paperclip.ing/reference/cli/common-options/) for how API base and context resolution work, and [Output and Scripting](https://docs.paperclip.ing/reference/cli/output-and-scripting/) for `--json` usage.
...
- [Agent Commands](https://docs.paperclip.ing/reference/cli/agent/) — set an agent's `adapterType` and `payload-json` using the schema you read here
- [What runs where](https://docs.paperclip.ing/guides/welcome/key-concepts/) — why adapters execute server-side and the CLI only triggers and observes
- [Common Options](https://docs.paperclip.ing/reference/cli/common-options/) — shared client flags and API base resolution

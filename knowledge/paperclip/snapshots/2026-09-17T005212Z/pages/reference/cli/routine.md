> Official reference snapshot. Source: https://docs.paperclip.ing/reference/cli/routine/
> Retrieved: 2026-09-17T00:52:20.458313+00:00
...
> **Note:** Like every other work object in Paperclip, the actual work a routine produces runs server-side. These commands create, configure, fire, and observe routines through the API — they do not execute the work themselves. See [What Runs Where](https://docs.paperclip.ing/guides/welcome/key-concepts/).
...
- [Run Commands](https://docs.paperclip.ing/reference/cli/run/) — inspect and control heartbeat runs, including the runs a routine spawns.
- [Issue Commands](https://docs.paperclip.ing/reference/cli/issue/) — the tasks routines create; filter by `originKind=routine_execution` to find them.
- [Common Options](https://docs.paperclip.ing/reference/cli/common-options/) — the client flags every `routine` command accepts.
- [Output And Scripting](https://docs.paperclip.ing/reference/cli/output-and-scripting/) — using `--json` to pipe routine data into other tools.
- [Routines And Automation](https://docs.paperclip.ing/guides/projects-workflow/routines/) — the end-to-end guide to designing automation.

> Official reference snapshot. Source: https://docs.paperclip.ing/reference/cli/secrets/
> Retrieved: 2026-09-17T00:52:21.053810+00:00
...
Every command here is company-scoped or operates on a specific secret/provider-config ID, and every command accepts the [common client options](https://docs.paperclip.ing/reference/cli/common-options/) (`--data-dir`, `--api-base`, `--api-key`, `--context`, `--profile`, `--json`). Company-scoped subcommands require `-C, --company-id <id>`.
...
- [Common Options](https://docs.paperclip.ing/reference/cli/common-options/) — the shared client flags every secrets command accepts.
- [Output and Scripting](https://docs.paperclip.ing/reference/cli/output-and-scripting/) — using `--json` to pipe secret metadata into other tools.
- [Agent Commands](https://docs.paperclip.ing/reference/cli/agent/) — agents whose env bindings reference these secrets.
- [Project Commands](https://docs.paperclip.ing/reference/cli/project/) — project env declarations that consume secrets.
- [Company Commands](https://docs.paperclip.ing/reference/cli/company/) — exports/imports that emit the env declarations `secrets declarations` previews.
- [Access Commands](https://docs.paperclip.ing/reference/cli/access/) — broader authorization and credential context.

> Official reference snapshot. Source: https://docs.paperclip.ing/reference/cli/goal/
> Retrieved: 2026-09-17T00:52:20.263011+00:00
...
- A **project** is created against one or more goals — `project create` takes `--goal-ids`. The project is the vehicle that moves those goals forward. See [Project Commands](https://docs.paperclip.ing/reference/cli/project/).
- An **issue** can carry a `goalId`, and when it doesn't, the server resolves a goal in order of precedence: the issue's own goal, then the project's goal, then the company's default goal. See [Issue Commands](https://docs.paperclip.ing/reference/cli/issue/).
...
All goal commands accept the common client options (`--data-dir`, `--api-base`, `--api-key`, `--context`, `--profile`, `--json`). The collection commands (`list`, `create`) are company-scoped and take `-C, --company-id <id>`; the single-goal commands (`get`, `update`, `delete`) address a goal directly by its ID. See [Common Options](https://docs.paperclip.ing/reference/cli/common-options/) for the shared flags and the API base resolution order.
...
- [Project Commands](https://docs.paperclip.ing/reference/cli/project/) — create projects against goals with `--goal-ids`
- [Issue Commands](https://docs.paperclip.ing/reference/cli/issue/) — set an issue's `goalId` and how goal precedence resolves
- [Company Commands](https://docs.paperclip.ing/reference/cli/company/) — select and inspect the company that owns the goals
- [Common Options](https://docs.paperclip.ing/reference/cli/common-options/) — shared flags and API base resolution
- [Output and Scripting](https://docs.paperclip.ing/reference/cli/output-and-scripting/) — using `--json` to script against goal data

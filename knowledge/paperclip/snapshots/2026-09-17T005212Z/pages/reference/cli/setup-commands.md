...
- create the initial config and local data directories ([`onboard`](https://docs.paperclip.ing/reference/cli/setup-commands/#paperclipai-onboard))
- verify and repair an install before or after a config change ([`doctor`](https://docs.paperclip.ing/reference/cli/setup-commands/#paperclipai-doctor))
- print the environment block a remote deployment will need ([`env`](https://docs.paperclip.ing/reference/cli/setup-commands/#paperclipai-env))
- change deployment mode, database, storage, secrets, or logging ([`configure`](https://docs.paperclip.ing/reference/cli/setup-commands/#paperclipai-configure))
- take a one-off database snapshot ([`db:backup`](https://docs.paperclip.ing/reference/cli/setup-commands/#paperclipai-dbbackup))
- trust a private hostname in authenticated mode ([`allowed-hostname`](https://docs.paperclip.ing/reference/cli/setup-commands/#paperclipai-allowed-hostname))
- bootstrap and launch a local server in one step ([`run`](https://docs.paperclip.ing/reference/cli/setup-commands/#paperclipai-run-local-bootstrap))
- keep an instance running in the background across logins ([`service`](https://docs.paperclip.ing/reference/cli/service/))
- install, update, roll back, or remove the CLI itself ([Installing the CLI](https://docs.paperclip.ing/reference/cli/installation/) and [Update Paperclip](https://docs.paperclip.ing/how-to/update-paperclip/))
- mint the first board credential headlessly ([`auth bootstrap-ceo`](https://docs.paperclip.ing/reference/cli/setup-commands/#paperclipai-auth-bootstrap-ceo))
- trigger one agent heartbeat for debugging ([`heartbeat run`](https://docs.paperclip.ing/reference/cli/setup-commands/#paperclipai-heartbeat-run))
- pause every routine in a company during an incident ([`routines disable-all`](https://docs.paperclip.ing/reference/cli/setup-commands/#paperclipai-routines-disable-all))

> **Note:** Examples use the installed binary `paperclipai`. Inside the monorepo you can substitute `pnpm paperclipai`; see [Installation](https://docs.paperclip.ing/reference/cli/installation/).

---

## `paperclipai run` (local bootstrap)

> **Warning:** `paperclipai run` with **no subcommand** is a setup command: it bootstraps and starts a local server. The `run` **subcommands** (`run list`, `run get`, `run events`, `run log`, `run cancel`, and friends) are a *different feature* — they inspect and control heartbeat runs over the API and are documented in [Runs](https://docs.paperclip.ing/reference/cli/run/). Do not confuse the two.
...
2. If no config exists and you are in a TTY, it hands off to [`onboard`](https://docs.paperclip.ing/reference/cli/setup-commands/#paperclipai-onboard) interactively. In a non-interactive shell it errors and tells you to run `onboard` first.
3. Runs [`doctor`](https://docs.paperclip.ing/reference/cli/setup-commands/#paperclipai-doctor) with repair enabled by default; if any check fails it stops without starting the server.
4. Starts the Paperclip server.
5. If the instance is in `authenticated` mode with the embedded database, it generates a bootstrap CEO invite after startup (see [`auth bootstrap-ceo`](https://docs.paperclip.ing/reference/cli/setup-commands/#paperclipai-auth-bootstrap-ceo)).
...
> **Tip:** `--bind` only matters on the very first `run`, when it forwards the preset to onboarding. Once a config exists, `run` reuses it; change binding later with [`configure --section server`](https://docs.paperclip.ing/reference/cli/setup-commands/#paperclipai-configure).

> **Warning:** Before anything else, `run` checks whether this instance is already active under the service manager. If it is, `run` refuses to start — two servers on one port and one database is not a state you want — and points you at `paperclipai service status --instance <id>`. `--force` bypasses that check when you genuinely want a foreground process anyway. See [Service](https://docs.paperclip.ing/reference/cli/service/).
...
> **Note:** If a valid config already exists, `onboard` preserves it unchanged, ensures the agent JWT secret and secrets key exist, and prints next-step commands. Use [`configure`](https://docs.paperclip.ing/reference/cli/setup-commands/#paperclipai-configure) to change settings on an existing install rather than re-onboarding.

When the instance is `authenticated` with the embedded database, the bootstrap CEO invite is deferred until the server is running — `onboard` reminds you to run `paperclipai run` then [`auth bootstrap-ceo`](https://docs.paperclip.ing/reference/cli/setup-commands/#paperclipai-auth-bootstrap-ceo).
...
| Managed install | Only runs if you have a [managed install](https://docs.paperclip.ing/reference/cli/installation/#the-managed-install). Confirms the manifest and active payload agree, that `~/.local/bin/paperclipai` is a real Paperclip shim, that its directory is on `PATH` (a warning if not), and that no orphaned payloads are left behind. |
| Background service | Only runs if a [service](https://docs.paperclip.ing/reference/cli/service/) is installed for this instance. Confirms the definition on disk matches what your current install would generate, that the service is active, that the health endpoint answers, and that the running version matches your managed install. On Linux it warns when start-on-login is on but systemd lingering is off. |
...
> **Note:** After this first credential exists, you no longer need `bootstrap-ceo`. A board token can mint further board tokens and agent keys non-interactively — see [Authentication](https://docs.paperclip.ing/reference/cli/authentication/).
...
The command polls run events and logs until the run reaches a terminal status (`succeeded`, `failed`, `cancelled`, `timed_out`) and exits non-zero on anything but success, printing error and result details. To inspect runs you triggered elsewhere, use the [`run`](https://docs.paperclip.ing/reference/cli/run/) subcommands.

> **Tip:** For routine agent wake-ups during normal operation, prefer [`agent wake`](https://docs.paperclip.ing/reference/cli/agent/) and the prompt-handoff flow in [`prompt`](https://docs.paperclip.ing/reference/cli/prompt/). Reach for `heartbeat run` when you specifically want to watch a single run unfold.
...
It reports how many routines it paused, how many were already paused, and how many were archived (left untouched). Routines in any state other than `paused`/`archived` are flipped to `paused`. To create, edit, or resume individual routines through the API, use the [`routine`](https://docs.paperclip.ing/reference/cli/routine/) commands.

> **Warning:** This pauses *all* of a company's routines at once. Re-enabling is a per-routine operation via [`routine`](https://docs.paperclip.ing/reference/cli/routine/); there is no single "enable-all" counterpart, so use this deliberately.
...
- [Installation](https://docs.paperclip.ing/reference/cli/installation/) — install the CLI and pick a deployment shape
- [Service](https://docs.paperclip.ing/reference/cli/service/) — run an instance in the background with `paperclipai service`
- [Update Paperclip](https://docs.paperclip.ing/how-to/update-paperclip/) — check, apply, and roll back new versions
- [Authentication](https://docs.paperclip.ing/reference/cli/authentication/) — board tokens, agent keys, and the login flows after bootstrap
- [Runs](https://docs.paperclip.ing/reference/cli/run/) — inspecting and controlling heartbeat runs (the `run` *subcommands*)
- [Routines](https://docs.paperclip.ing/reference/cli/routine/) — create, edit, and resume scheduled automation
- [Agents](https://docs.paperclip.ing/reference/cli/agent/) — waking agents and `agent local-cli` for local execution
- [Common options](https://docs.paperclip.ing/reference/cli/common-options/) — shared flags and context/profile resolution

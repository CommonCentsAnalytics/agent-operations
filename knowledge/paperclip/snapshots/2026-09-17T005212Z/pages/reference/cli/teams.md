> Official reference snapshot. Source: https://docs.paperclip.ing/reference/cli/teams/
> Retrieved: 2026-09-17T00:52:20.948585+00:00
...
> **Tip:** Add `--json` to any command to print the raw API result for scripting. Browsing commands need no company context, but `list`, `preview`, and `install` operate against a company and accept `--company-id <id>` (and respect your selected profile). See [common options](https://docs.paperclip.ing/reference/cli/common-options/).
...
- [Team Catalog guide](https://docs.paperclip.ing/guides/org/team-catalog/) — what catalog teams are and how to use them from the app.
- [Skills commands](https://docs.paperclip.ing/reference/cli/skills/) — the skills catalog, the closest single-capability analogue.
- [Company commands](https://docs.paperclip.ing/reference/cli/company/) — company context, export, and import (catalog installs ride on the same portability engine).
- [Agent commands](https://docs.paperclip.ing/reference/cli/agent/) — inspect the agents an install creates.
- [Approval commands](https://docs.paperclip.ing/reference/cli/approval/) — work with the board approval the forbidden-install fallback raises.

> Official reference snapshot. Source: https://docs.paperclip.ing/reference/cli/approval/
> Retrieved: 2026-09-17T00:52:20.580516+00:00
...
Every subcommand accepts the [common client options](https://docs.paperclip.ing/reference/cli/common-options/) (`--data-dir`, `--api-base`, `--api-key`, `--context`, `--profile`, `--json`). The company-scoped subcommands (`list`, `create`) additionally require `-C, --company-id`. Single-approval subcommands operate on an approval ID and do not need a company.
...
- [Common Options](https://docs.paperclip.ing/reference/cli/common-options/) — the shared client flags (`--company-id`, `--api-base`, `--api-key`, `--json`, and the rest) accepted by every approval subcommand.
- [Output and Scripting](https://docs.paperclip.ing/reference/cli/output-and-scripting/) — how `--json` output is shaped for pipelines.
- [Issue Commands](https://docs.paperclip.ing/reference/cli/issue/) — link approvals to the work that motivated them with the issue approval link commands.
- [Agent Commands](https://docs.paperclip.ing/reference/cli/agent/) — the agents that request hires and run the work an approval gates.
- [Activity](https://docs.paperclip.ing/reference/cli/activity/) — the audit trail where approval decisions are recorded.

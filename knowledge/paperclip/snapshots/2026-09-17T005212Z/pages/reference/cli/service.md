> Official reference snapshot. Source: https://docs.paperclip.ing/reference/cli/service/
> Retrieved: 2026-09-17T00:52:19.793623+00:00
...
> **Note:** Every `service` subcommand accepts `-i, --instance <id>` to target a specific local instance (default `default`), and `--json` to print machine-readable output instead of text. If service management is not available — Windows, or a Linux container or WSL1 without a usable systemd user manager — each command says so and exits cleanly, and you should use [`paperclipai run`](https://docs.paperclip.ing/reference/cli/setup-commands/#paperclipai-run-local-bootstrap) instead.

---

## What gets installed

The service definition is a small file in your own home directory that tells the OS to run `paperclipai run --instance <id>`. It is generated from your current install, so it always points at the CLI you actually have.

|  | macOS | Linux |
| --- | --- | --- |
| Supervisor | launchd | systemd (user) |
| Service name | `ing.paperclip.paperclipai` | `paperclipai.service` |
| Definition file | `~/Library/LaunchAgents/ing.paperclip.paperclipai.plist` | `~/.config/systemd/user/paperclipai.service` |
| Logs | `~/.paperclip/instances/<id>/logs/service.log` and `service.err.log` | the journal, via `journalctl --user` |

Non-default instances get their own service so they never collide: instance `dev` becomes `ing.paperclip.paperclipai.dev` on macOS and `paperclipai-dev.service` on Linux.

The service is started with `PAPERCLIP_SERVICE_MANAGED=1`, `PAPERCLIP_INSTANCE_ID`, and `PAPERCLIP_HOME` set, so the server knows it is running under a supervisor and which instance and home directory to use.

---

## Install the service

`service install` writes the definition, registers it with the supervisor, and starts Paperclip.

```sh
paperclipai service install
paperclipai service install --no-start-now
paperclipai service install --no-start-on-login
...
You can also get all of this during first-run setup: [`paperclipai onboard`](https://docs.paperclip.ing/reference/cli/setup-commands/#paperclipai-onboard) offers to install the service for you, and `paperclipai onboard --install-service` skips the question.
...
Those can disagree, and the disagreement is the useful part: a service that is `active` but not healthy is a server that started and then failed, and a healthy instance while the service is inactive usually means a foreground `paperclipai run` is holding the port.
...
paperclipai service restart --expected-version 2026.609.0
```
...
`paperclipai update` performs this same hot restart for you when it detects an active service, so you rarely need to run it by hand after an update. See [Update Paperclip](https://docs.paperclip.ing/how-to/update-paperclip/).
...
paperclipai service logs --lines 500
```
...
> **Tip:** `paperclipai service logs --follow` in one window while you trigger work in another is the quickest way to watch a startup problem happen in real time.
...
This removes only the service. Your Paperclip install, config, and data are untouched, and `paperclipai run` still works exactly as before. To remove the CLI itself, see [`paperclipai uninstall`](https://docs.paperclip.ing/reference/cli/installation/#removing-the-managed-install) — which stops and removes the service for you as part of its own cleanup.

---

## How doctor sees the service

[`paperclipai doctor`](https://docs.paperclip.ing/reference/cli/setup-commands/#paperclipai-doctor) checks the service as part of its normal run, so you usually find out something is wrong before you go looking. It verifies that the definition on disk matches what the current install would generate, that the service is active, that the health endpoint reports OK, and that the running version matches your managed install. On Linux it also warns when start-on-login is enabled but lingering is off — the combination that quietly stops Paperclip when you log out.

If no service is installed, doctor passes and says so: running Paperclip in the foreground is a perfectly valid choice.

---

## See also

- [Installing the CLI](https://docs.paperclip.ing/reference/cli/installation/) — the managed install the service runs, and how to remove it.
- [Setup Commands](https://docs.paperclip.ing/reference/cli/setup-commands/) — `run`, `onboard`, and `doctor`, including the flags that interact with the service.
- [Update Paperclip](https://docs.paperclip.ing/how-to/update-paperclip/) — updating an instance that runs as a service.
- [Common Options](https://docs.paperclip.ing/reference/cli/common-options/) — the shared flags used across the CLI.

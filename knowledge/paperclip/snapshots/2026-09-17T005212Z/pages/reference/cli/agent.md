...
All commands accept the common client flags: `--data-dir <path>`, `--api-base <url>`, `--api-key <token>`, `--context <path>`, `--profile <name>`, and `--json`. Company-scoped commands also take `-C, --company-id <id>`. See [Common Options](https://docs.paperclip.ing/reference/cli/common-options/) for how those resolve.
...
> **Tip:** Building the payload by hand is error-prone. The `paperclip-create-agent` skill inspects adapter options and drafts a valid config for you. The board can review and resolve a hire request with the [approval](https://docs.paperclip.ing/reference/cli/approval/) commands.
...
The command resolves the agent, then POSTs to `/api/agents/{id}/wakeup`. The server runs the adapter; the CLI prints the wakeup response. To stream and inspect the resulting run, use the [`run`](https://docs.paperclip.ing/reference/cli/run/) subcommands.
...
> **Note:** Stocking a company's skill *library* and authoring skills is a separate concern — see the [skills](https://docs.paperclip.ing/reference/cli/skills/) reference. `agent skills:sync` only chooses which library skills a given agent should use.
...
> **Warning:** The minted API key's plaintext token is shown **once**, in the command output. The exports embed it directly. Treat the output as a secret, and revoke the key with `token agent revoke` when the local session is done. See [Authentication](https://docs.paperclip.ing/reference/cli/authentication/).
...
- [Authentication](https://docs.paperclip.ing/reference/cli/authentication/) — board tokens, agent keys, and how `connect` mints persona-aware profiles
- [Common Options](https://docs.paperclip.ing/reference/cli/common-options/) — shared flags and API base resolution
- [Run](https://docs.paperclip.ing/reference/cli/run/) — inspect and control the heartbeat runs that `agent wake` triggers
- [Prompt](https://docs.paperclip.ing/reference/cli/prompt/) — hand work to an agent as a task-and-comment via `agent prompt`
- [Skills](https://docs.paperclip.ing/reference/cli/skills/) — browse the catalog, stock the company library, and author skills
- [Approval](https://docs.paperclip.ing/reference/cli/approval/) — resolve hire requests and other board-gated actions
- [Company](https://docs.paperclip.ing/reference/cli/company/) — the company context every company-scoped agent command needs

> Official reference snapshot. Source: https://docs.paperclip.ing/reference/cli/asset/
> Retrieved: 2026-09-17T00:52:21.147341+00:00
...
All three subcommands accept the [common client options](https://docs.paperclip.ing/reference/cli/common-options/) (`--data-dir`, `--api-base`, `--api-key`, `--context`, `--profile`, `--json`). The two upload commands are company-scoped and take `-C, --company-id <id>`; `asset content` works on a global asset id and is not company-scoped.
...
API base resolution follows the standard order (`--api-base`, then `PAPERCLIP_API_URL`, then your selected context profile, then the local config port, then `http://localhost:3100`). If a connection fails, the error names the URL it tried so you can confirm you are pointed at the right instance — see [common options](https://docs.paperclip.ing/reference/cli/common-options/) and [output and scripting](https://docs.paperclip.ing/reference/cli/output-and-scripting/) for the full conventions.

---

## See also

- [Common Options](https://docs.paperclip.ing/reference/cli/common-options/) — the flags every client command shares, including `--company-id` and API base resolution
- [Output and Scripting](https://docs.paperclip.ing/reference/cli/output-and-scripting/) — how `--json` shapes results and how to consume them
- [Skills Commands](https://docs.paperclip.ing/reference/cli/skills/) — managing a company's skill library, another company-scoped file workflow
- [Adapter Commands](https://docs.paperclip.ing/reference/cli/adapter/) — configuring the runtime adapters that execute agent work
- [Company Commands](https://docs.paperclip.ing/reference/cli/company/) — inspecting and exporting the company that owns these assets

> Official reference snapshot. Source: https://docs.paperclip.ing/reference/cli/dashboard/
> Retrieved: 2026-09-17T00:52:20.716556+00:00
...
This command is company-scoped, so it needs a company. It does not accept a positional company argument — pass the company with `-C` / `--company-id`, or let it resolve from your environment or profile (see [Resolving the company](https://docs.paperclip.ing/reference/cli/dashboard/#resolving-the-company)).
...
These are the standard client flags shared by every company-scoped CLI command. For the full explanation of how authentication, context, and the API base are resolved, see [Common options](https://docs.paperclip.ing/reference/cli/common-options/).
...
Monetary values are in cents — divide by 100 for dollars. The `costs` block is the same data you can break down further with the [`cost`](https://docs.paperclip.ing/reference/cli/cost/) commands; the dashboard gives you the headline number, `cost` gives you the attribution.
...
For the full set of scripting conventions — exit codes, `--json` behavior, and how to chain CLI calls — see [Output and scripting](https://docs.paperclip.ing/reference/cli/output-and-scripting/).
...
2. If `pendingApprovals` is non-zero, follow up with [`approval list`](https://docs.paperclip.ing/reference/cli/approval/).
3. If `agents.error` or `budgets.pausedAgents` is non-zero, follow up with [`agent`](https://docs.paperclip.ing/reference/cli/agent/) and [`activity list`](https://docs.paperclip.ing/reference/cli/activity/) to see why.
4. If `costs.monthUtilizationPercent` is climbing toward the budget, break it down with [`cost`](https://docs.paperclip.ing/reference/cli/cost/).

The dashboard tells you *whether* to look closer; the scoped commands tell you *what* happened.

> **Note:** `dashboard get` is a read of the current company state, not a live stream. To watch heartbeat runs as they execute, use the [`run`](https://docs.paperclip.ing/reference/cli/run/) inspection subcommands; to follow the audit trail, use [`activity`](https://docs.paperclip.ing/reference/cli/activity/).

---

## See also

- [Common options](https://docs.paperclip.ing/reference/cli/common-options/) — authentication, context, and API base resolution shared by every client command
- [Output and scripting](https://docs.paperclip.ing/reference/cli/output-and-scripting/) — `--json` conventions and chaining CLI calls
- [Cost commands](https://docs.paperclip.ing/reference/cli/cost/) — break the headline spend down by agent, model, provider, biller, or project
- [Approval commands](https://docs.paperclip.ing/reference/cli/approval/) — review and resolve the pending approvals the dashboard counts
- [Activity commands](https://docs.paperclip.ing/reference/cli/activity/) — the audit trail behind agent and run state
- [Agent commands](https://docs.paperclip.ing/reference/cli/agent/) — inspect the agents the dashboard summarizes
- [Run commands](https://docs.paperclip.ing/reference/cli/run/) — inspect heartbeat runs behind the `runActivity` series

> Official reference snapshot. Source: https://docs.paperclip.ing/reference/cli/issue/
> Retrieved: 2026-09-17T00:52:20.226665+00:00
...
> **Note:** All `issue` commands accept the [common client flags](https://docs.paperclip.ing/reference/cli/common-options/) — `--api-base`, `--api-key`, `--context`, `--profile`, `--data-dir`, and `--json`. Company-scoped commands additionally take `-C, --company-id <id>`. Those flags are not repeated in every table below.
...
Link existing approvals to an issue, list them, and unlink. To create approvals themselves, see the [approval commands](https://docs.paperclip.ing/reference/cli/approval/).
...
These read the heartbeat runs tied to an issue — what executed, what is in flight, and what is active right now. These are the server-side runs an issue produced; they are distinct from the local `run` bootstrap command. See the [run commands](https://docs.paperclip.ing/reference/cli/run/) for the run surface itself.
...
Feedback captures votes and traces against an issue and its targets — the raw material for evaluating agent output. See the [feedback commands](https://docs.paperclip.ing/reference/cli/feedback/) for the broader feedback surface.
...
- [Common options](https://docs.paperclip.ing/reference/cli/common-options/) — flags shared by every client command.
- [Output and scripting](https://docs.paperclip.ing/reference/cli/output-and-scripting/) — using `--json` in pipelines.
- [Approval commands](https://docs.paperclip.ing/reference/cli/approval/) — create and decide the approvals you link here.
- [Run commands](https://docs.paperclip.ing/reference/cli/run/) — inspect and control heartbeat runs.
- [Feedback commands](https://docs.paperclip.ing/reference/cli/feedback/) — the wider feedback and trace surface.
- [Goal commands](https://docs.paperclip.ing/reference/cli/goal/) and [Project commands](https://docs.paperclip.ing/reference/cli/project/) — the containers issues attach to.

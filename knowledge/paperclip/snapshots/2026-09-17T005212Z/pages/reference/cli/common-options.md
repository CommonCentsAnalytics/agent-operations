> Official reference snapshot. Source: https://docs.paperclip.ing/reference/cli/common-options/
> Retrieved: 2026-09-17T00:52:19.627589+00:00
...
| `--json` | Emit raw JSON instead of the human-readable inline format. See [Output & Scripting](https://docs.paperclip.ing/reference/cli/output-and-scripting/). |
...
The two personas matter: a `board` profile carries full board authority across the instance, while an `agent` profile is scoped to exactly one company and one agent. See [Authentication](https://docs.paperclip.ing/reference/cli/authentication/) for how each credential type is minted.
...
- [Authentication](https://docs.paperclip.ing/reference/cli/authentication/) — minting board tokens and agent keys, and the connect wizard
- [Output & Scripting](https://docs.paperclip.ing/reference/cli/output-and-scripting/) — `--json` and machine-readable output
- [Installation](https://docs.paperclip.ing/reference/cli/installation/) — installing the `paperclipai` binary

> Official reference snapshot. Source: https://docs.paperclip.ing/reference/cli/feedback/
> Retrieved: 2026-09-17T00:52:21.248753+00:00
...
All four subcommands accept the common client options described in [Common Options](https://docs.paperclip.ing/reference/cli/common-options/): `--data-dir`, `--api-base`, `--api-key`, `--context`, `--profile`, and `--json`. `report` and `export` are company-scoped and additionally take `-C, --company-id <id>`.
...
- Use `--json` on every subcommand when you are piping into `jq` or another tool. See [Output and Scripting](https://docs.paperclip.ing/reference/cli/output-and-scripting/) for the conventions.
...
- [Issue Commands](https://docs.paperclip.ing/reference/cli/issue/) — feedback traces are anchored to issues; use these to inspect the work that was voted on.
- [Company Commands](https://docs.paperclip.ing/reference/cli/company/) — feedback is company-scoped; export and portability live here.
- [Common Options](https://docs.paperclip.ing/reference/cli/common-options/) — the shared client flags every feedback subcommand accepts.
- [Output and Scripting](https://docs.paperclip.ing/reference/cli/output-and-scripting/) — how `--json` output is structured for automation.

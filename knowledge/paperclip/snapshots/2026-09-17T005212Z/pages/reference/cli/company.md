> Official reference snapshot. Source: https://docs.paperclip.ing/reference/cli/company/
> Retrieved: 2026-09-17T00:52:20.052042+00:00
...
Every command on this page accepts the standard client options: `--data-dir <path>`, `--api-base <url>`, `--api-key <token>`, `--context <path>`, `--profile <name>`, and `--json`. See [Common Options](https://docs.paperclip.ing/reference/cli/common-options/) for how the API base and credentials are resolved.
...
`company archive` posts to `/api/companies/{id}/archive` and takes no payload. Archiving is the reversible way to take a company out of active rotation. Reach for [`company delete`](https://docs.paperclip.ing/reference/cli/company/#delete-a-company) only when you genuinely need the records gone and the server is configured to permit it.
...
> **Note:** This pair of commands is how you move a company between instances. It replaces the retired host-to-host Cloud Sync, so there is no longer any need to connect one instance to another first. See [Export & Import](https://docs.paperclip.ing/guides/power/export-import/) for the walkthrough.
...
> **Warning:** There is no undo. If you only need to take a company out of rotation, use [`company archive`](https://docs.paperclip.ing/reference/cli/company/#archive) instead.
...
- [Common Options](https://docs.paperclip.ing/reference/cli/common-options/) — API base resolution, credentials, and the flags every command shares
- [Output and Scripting](https://docs.paperclip.ing/reference/cli/output-and-scripting/) — using `--json` and piping CLI output
- [Project Commands](https://docs.paperclip.ing/reference/cli/project/) — manage the projects inside a company
- [Agent Commands](https://docs.paperclip.ing/reference/cli/agent/) — inspect agents and set up a local agent CLI session
- [Feedback Commands](https://docs.paperclip.ing/reference/cli/feedback/) — work with feedback traces beyond the company-scoped views
- [Your First Company via CLI](https://docs.paperclip.ing/guides/getting-started/your-first-company/) — end-to-end walkthrough

> Official reference snapshot. Source: https://docs.paperclip.ing/reference/cli/connect/
> Retrieved: 2026-09-17T00:52:20.036025+00:00
...
> **Note:** `connect` is interactive by design. It needs a real terminal (a TTY) on both input and output. In scripts and CI it will refuse to run and tell you to pass `--api-base`/`--api-key` or use the `context` and token commands instead. See [Common Options](https://docs.paperclip.ing/reference/cli/common-options/) for the non-interactive path.
...
1. **Confirm the API base.** It pre-fills the API base it would otherwise resolve (see [Common Options](https://docs.paperclip.ing/reference/cli/common-options/) for that resolution order) and lets you accept or change it. The placeholder is `http://localhost:3100`.
...
A board profile carries board authority across the instance; an agent profile is bound to exactly one company and one agent. This is the same persona split described under [Context profiles](https://docs.paperclip.ing/reference/cli/common-options/). For how each credential type is minted and what it can do, see [Authentication](https://docs.paperclip.ing/reference/cli/authentication/).
...
| Flag | Default | Use |
| --- | --- | --- |
| `--persona <persona>` | *(prompted)* | The persona to configure: `board` or `agent`. Pass it to skip the persona question. |
...
> **Tip:** The profile records the *name* of the env var, not the secret. After connecting, keep the token in your environment (the printed exports do exactly this) so your context file stays safe to share. This is the same `apiKeyEnvVarName` mechanism explained in [Common Options](https://docs.paperclip.ing/reference/cli/common-options/).
...
`connect` accepts the standard client options — `--data-dir <path>`, `--api-base <url>`, `--api-key <token>`, `--context <path>`, `--profile <name>`, `-C, --company-id <id>`, and `--json`. Of these, `--api-base`, `--context`, `--profile`, and `--company-id` seed the wizard's starting values (the API base it pre-fills, the context file it writes, the profile name it suggests, and the company it pre-selects). See [Common Options](https://docs.paperclip.ing/reference/cli/common-options/) for how API base, context, and company scope are resolved, and [Output and Scripting](https://docs.paperclip.ing/reference/cli/output-and-scripting/) for `--json` usage.
...
- [Authentication](https://docs.paperclip.ing/reference/cli/authentication/) — how board tokens and agent keys are minted and what each can do
- [Common Options](https://docs.paperclip.ing/reference/cli/common-options/) — shared client flags, context profiles, and API base resolution
- [Output and Scripting](https://docs.paperclip.ing/reference/cli/output-and-scripting/) — working with `--json` output
- [Overview](https://docs.paperclip.ing/reference/cli/overview/) — the CLI's place in the operating model

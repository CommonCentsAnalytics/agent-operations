> Official reference snapshot. Source: https://docs.paperclip.ing/reference/cli/activity/
> Retrieved: 2026-09-17T00:52:20.666409+00:00
...
All three subcommands accept the [common client options](https://docs.paperclip.ing/reference/cli/common-options/) (`--api-base`, `--api-key`, `--context`, `--profile`, `--data-dir`, `--json`). `activity list` and `activity create` are company-scoped and require `--company-id`; `activity issue` resolves the company from the issue itself.
...
- [Issue Commands](https://docs.paperclip.ing/reference/cli/issue/) — create, update, comment on, and check out the tasks whose history shows up here.
- [Dashboard Commands](https://docs.paperclip.ing/reference/cli/dashboard/) — the same top-level company state the UI surfaces, alongside the activity feed.
- [Common Options](https://docs.paperclip.ing/reference/cli/common-options/) — the shared flags every client command accepts.
- [Output and Scripting](https://docs.paperclip.ing/reference/cli/output-and-scripting/) — how `--json` output is shaped for piping into other tools.

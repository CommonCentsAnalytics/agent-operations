> Official reference snapshot. Source: https://docs.paperclip.ing/reference/cli/skills/
> Retrieved: 2026-09-17T00:52:20.940502+00:00
...
> **Tip:** Add `--json` to any command to print the raw API result for scripting. Every company-scoped command takes `--company-id <company-id>` (and respects your selected profile/context). See [common options](https://docs.paperclip.ing/reference/cli/common-options/).
...
After install the command reminds you that nothing is attached yet. To make an agent use it, run an [agent attach](https://docs.paperclip.ing/reference/cli/skills/#agent-attach-desired-skills) afterward.
...
- [Adapter commands](https://docs.paperclip.ing/reference/cli/adapter/) — the runtimes that reconcile desired skills into files on disk.
- [Company commands](https://docs.paperclip.ing/reference/cli/company/) — company context, export, and import (skills travel in company packages).
- [Project commands](https://docs.paperclip.ing/reference/cli/project/) — the projects and workspaces `scan-projects` reads from.
- [Common options](https://docs.paperclip.ing/reference/cli/common-options/) and [output and scripting](https://docs.paperclip.ing/reference/cli/output-and-scripting/) — shared flags and `--json` handling.

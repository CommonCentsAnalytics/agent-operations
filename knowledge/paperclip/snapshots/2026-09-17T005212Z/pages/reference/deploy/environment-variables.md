> Official reference snapshot. Source: https://docs.paperclip.ing/reference/deploy/environment-variables/
> Retrieved: 2026-09-17T00:52:23.879735+00:00
...
In the UI, every key the overlay controls renders with a lock badge reading **Managed by Paperclip Cloud** and a disabled toggle. Self-hosted responses carry no managed keys at all, so every toggle stays editable. See [Experimental features](https://docs.paperclip.ing/experimental/overview/#if-a-toggle-is-locked) for the user-facing view.
...
- **`config`** is stored verbatim on the environment row and validated by the provider plugin when it acquires a lease. See [Sandbox Providers](https://docs.paperclip.ing/reference/adapters/sandbox-providers/) for the fields each provider accepts.
...
Dropping the `environments` section, or the entry inside it, stops future refreshes — it does not delete or archive the environment row. This matches how `plugins.autoInstall` behaves ([Plugins](https://docs.paperclip.ing/administration/plugins/#what-it-wont-undo) covers the plugin side): runs may still hold leases against that environment, so withdrawing it is an explicit action someone takes, not a side effect of an edited document. The archiving described above is scoped to a provider that was declared but couldn't come up, never to an entry you removed.
...
These values are covered in more detail in [Secrets](https://docs.paperclip.ing/reference/deploy/secrets/).
...
| Variable | Always set? |
| --- | --- |
| `PAPERCLIP_AGENT_ID` | yes |
| `PAPERCLIP_COMPANY_ID` | yes |
| `PAPERCLIP_API_URL` | yes |
| `PAPERCLIP_API_KEY` | local adapters |
| `PAPERCLIP_RUN_ID` | yes |
| `PAPERCLIP_TASK_ID` | wake-driven |
| `PAPERCLIP_WAKE_REASON` | wake-driven |
| `PAPERCLIP_WAKE_COMMENT_ID` | comment wakes |
| `PAPERCLIP_WAKE_PAYLOAD_JSON` | some adapters |
| `PAPERCLIP_APPROVAL_ID` | approval wakes |
| `PAPERCLIP_APPROVAL_STATUS` | approval wakes |
| `PAPERCLIP_LINKED_ISSUE_IDS` | optional |
...
| --- | --- | --- |
| `PAPERCLIP_CODEX_PROVIDERS` | `codex_local` | JSON of custom providers (and an optional `model_provider`) written into Codex's managed `config.toml`. See [Codex](https://docs.paperclip.ing/reference/adapters/codex/). |
| `PAPERCLIP_PI_PROVIDERS` | `pi_local` | JSON of custom providers written into Pi's managed `models.json`. See [Pi](https://docs.paperclip.ing/reference/adapters/pi/). |
| `PAPERCLIP_OPENCODE_PROVIDERS` | `opencode_local` | JSON merged into OpenCode's `provider` config. See [OpenCode](https://docs.paperclip.ing/reference/adapters/opencode/). |
| `PAPERCLIP_OPENCODE_SMALL_MODEL` | `opencode_local` | Sets OpenCode's `small_model` (the auxiliary/helper model). See [OpenCode](https://docs.paperclip.ing/reference/adapters/opencode/). |

> Official reference snapshot. Source: https://docs.paperclip.ing/reference/plugins/sdk/
> Retrieved: 2026-09-17T00:52:23.411444+00:00
...
This page is for **plugin authors**: the developers writing the code that ships inside a plugin package. If you only run plugins — install, configure, enable, disable — you want [Administration → Plugins](https://docs.paperclip.ing/administration/plugins/) instead.
...
- **Teaching Paperclip a new AI runtime.** Use an [adapter](https://docs.paperclip.ing/reference/adapters/creating-an-adapter/) instead — adapters speak the per-run wire protocol; plugins extend the server.
- **Adding instructions an agent should follow.** Write a [company skill](https://docs.paperclip.ing/how-to/write-a-company-skill/) — those are markdown an agent loads at run time, not server code.
...
- `@paperclipai/plugin-sdk/ui` — UI-bundle surface for plugin UI contributions. Out of scope for this page; see [Administration → Plugins](https://docs.paperclip.ing/administration/plugins/) for the operator-facing view.
...
| `ctx.execution` | Stream live output from a long-running `execute` call — see [Streaming live command output](https://docs.paperclip.ing/reference/plugins/sdk/#streaming-live-command-output). The interface is named `PluginExecutionClient` in the SDK source but is not currently re-exported as a name; it is reachable from `PluginContext`. |
...
| `PluginObjectReferenceProviderDeclaration` (+ `PluginObjectReferenceRefreshPolicy`) | External-object reference provider — see [External-object reference providers](https://docs.paperclip.ing/reference/plugins/sdk/#external-object-reference-providers). |
...
For the full manifest example and authoring rules, see the parent `doc/plugins/PLUGIN_AUTHORING_GUIDE.md`; the declaration types listed under [Manifest types](https://docs.paperclip.ing/reference/plugins/sdk/#manifest-types) above are the source of truth for what each managed entry accepts.
...
`PLUGIN_RPC_ERROR_CODES` includes `CROSS_TENANT_CONFIG`, the code the worker raises when a `configChanged` delivery would collapse a single-tenant worker onto a second company's configuration — see [Declaring multi-company support with `multiCompanyConfig`](https://docs.paperclip.ing/reference/plugins/sdk/#declaring-multi-company-support-with-multicompanyconfig).
...
- [Administration → Plugins](https://docs.paperclip.ing/administration/plugins/) — installing, enabling, configuring, and uninstalling plugins as an operator.
- [How-to → Write a Company Skill](https://docs.paperclip.ing/how-to/write-a-company-skill/) — instructions an agent loads, **not** server code.
- [Reference → Creating an Adapter](https://docs.paperclip.ing/reference/adapters/creating-an-adapter/) — the right extension point for new AI runtimes.
- [Reference → Skills](https://docs.paperclip.ing/reference/skills/) — the skill file shape and install pipeline a plugin's `PluginManagedSkillDeclaration` slots into.

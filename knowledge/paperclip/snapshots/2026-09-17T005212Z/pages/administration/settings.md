> Official reference snapshot. Source: https://docs.paperclip.ing/administration/settings/
> Retrieved: 2026-09-17T00:52:17.531429+00:00
...
Company and instance settings now share one navigation. The instance-wide **General** controls described below live on the organization's own **General** page (see [Company Administration](https://docs.paperclip.ing/administration/company/#general-settings)), so instance admins reach them from the same place as the organization settings rather than a separate area.
...
[Image reference: Profile settings](https://docs.paperclip.ing/administration/settings/user-guides/screenshots/dark/settings/profile.png)
...
[Image reference: Instance general settings](https://docs.paperclip.ing/administration/settings/user-guides/screenshots/dark/settings/instance-general.png)
...
These fields are informational. To change them you change the deployment itself — see [Environment Variables](https://docs.paperclip.ing/reference/deploy/environment-variables/).
...
[Image reference: Instance access](https://docs.paperclip.ing/administration/settings/user-guides/screenshots/dark/settings/instance-access.png)
...
[Image reference: Instance adapters](https://docs.paperclip.ing/administration/settings/user-guides/screenshots/dark/settings/instance-adapters.png)
...
For the full catalog of adapters Paperclip supports out of the box, along with the per-agent fields each one expects, see the reference docs: [Adapters overview](https://docs.paperclip.ing/reference/adapters/overview/).

---

## Instance: Experimental

[Image reference: Experimental flags](https://docs.paperclip.ing/administration/settings/user-guides/screenshots/dark/settings/experimental.png)
...
Each flag has its own page in the [Experimental](https://docs.paperclip.ing/experimental/overview/) section with motivation, setup, and usage instructions. In brief:

- **[Enable Environments](https://docs.paperclip.ing/experimental/environments/)** — environment management in instance settings, plus project and agent environment assignment (including custom sandbox images).
- **[Enable Isolated Workspaces](https://docs.paperclip.ing/experimental/isolated-workspaces/)** — execution-workspace controls in project configuration and isolated (git-worktree) checkouts for task runs. If you're not using isolated workspaces, leave this off; turning it on exposes extra UI you don't need.
- **[Experimental File Viewer](https://docs.paperclip.ing/experimental/file-viewer/)** — task-detail controls for browsing and previewing workspace files.
- **[Enable External Objects](https://docs.paperclip.ing/experimental/external-objects/)** — detects external URLs in issues and shows live status for referenced pull requests and tickets.
- **[Task Plan Decomposition Panel](https://docs.paperclip.ing/experimental/plan-decomposition-panel/)** — accepted-plan decomposition history on task detail pages, for debugging sub-task creation.
- **[Task Watchdogs](https://docs.paperclip.ing/experimental/task-watchdogs/)** — per-task watchdog agents that verify stopped task subtrees and restore live paths.
- **[Cloud Sync](https://docs.paperclip.ing/experimental/cloud-sync/)** — **retired.** Removed upstream together with its experimental toggle, so it no longer appears in this list on a current build. To move a company between instances, use [company Import/Export](https://docs.paperclip.ing/how-to/back-up-and-restore-a-company/).
- **[Server Info Debug View](https://docs.paperclip.ing/experimental/server-info-debug-view/)** — a "Server" section in the account drawer with restart time, running commit, and checkout state.
- **[Auto-Restart Dev Server When Idle](https://docs.paperclip.ing/experimental/auto-restart-dev-server/)** — only relevant under `pnpm dev:once`; restarts a stale dev boot once all local agent runs finish. For development on Paperclip itself, not production.
- **[Auto-Create Recovery Tasks](https://docs.paperclip.ing/experimental/auto-create-recovery-tasks/)** — lets the heartbeat scheduler create recovery tasks for stalled task dependency chains, with a lookback window and preview.
...
- [Heartbeats & Routines](https://docs.paperclip.ing/guides/projects-workflow/routines/) — the full picture of timer vs. event-driven wakes, and how to configure individual agents.
- [Environment Variables](https://docs.paperclip.ing/reference/deploy/environment-variables/) — the deployment-level settings that the General page reports on (auth mode, bootstrap state, exposure).

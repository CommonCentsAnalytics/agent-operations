> Official reference snapshot. Source: https://docs.paperclip.ing/administration/plugins/
> Retrieved: 2026-09-17T00:52:17.748677+00:00
...
[Image reference: Plugin list](https://docs.paperclip.ing/administration/plugins/user-guides/screenshots/dark/plugins/list.png)
...
[Image reference: Install a plugin](https://docs.paperclip.ing/administration/plugins/user-guides/screenshots/dark/plugins/install.png)
...
- **First-party plugins** — real plugins like the [LLM Wiki](https://docs.paperclip.ing/reference/plugins/llm-wiki/) when they're vendored into your checkout. These are also published to npm, so on instances that don't bundle them you'd install by full package name instead; see *By npm package name* below.
...
Once provisioned, these behave like any other installed plugin: same status badges, same detail page, same Configuration tab. You still choose whether and how to use one, by configuring a sandbox environment under **Settings → Instance settings → Environments**. See [Sandbox Providers](https://docs.paperclip.ing/reference/adapters/sandbox-providers/) for each provider's fields.
...
On a cloud-managed instance the list comes from the fleet instead, as part of the managed configuration document the harness hands the instance ([Cloud-managed instances](https://docs.paperclip.ing/reference/deploy/environment-variables/#cloud-managed-instances) has the details). That list is checked strictly: a key that isn't in the table above, or a bundle path that resolves outside the catalog root, makes the instance refuse to start rather than load something unexpected into the host. Failing loudly at boot is the point.
...
[Image reference: Plugin detail page](https://docs.paperclip.ing/administration/plugins/user-guides/screenshots/dark/plugins/detail.png)
...
[Image reference: Plugin settings](https://docs.paperclip.ing/administration/plugins/user-guides/screenshots/dark/plugins/settings.png)
...
[Image reference: Plugin jobs log](https://docs.paperclip.ing/administration/plugins/user-guides/screenshots/dark/plugins/jobs-log.png)
...
- The Paperclip CLI (`paperclipai`) wraps common authoring tasks — scaffolding, dev serving, building, and publishing. See the [CLI overview](https://docs.paperclip.ing/guides/getting-started/your-first-agent/) and the plugin adapter documentation in the main Paperclip repository for details.
...
See **[Develop a plugin locally](https://docs.paperclip.ing/how-to/develop-a-plugin-locally/)** for the full walkthrough — scaffolding flags, the reload semantics, when to switch back to an npm install, and the troubleshooting checklist for the moments edits stop reaching Paperclip.
...
If you're looking to extend what an individual agent can *do* rather than what Paperclip can *show*, see [Skills](https://docs.paperclip.ing/guides/org/skills/) and [Agent Adapters](https://docs.paperclip.ing/guides/org/agent-adapters/) next.

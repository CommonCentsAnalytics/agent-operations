> Official reference snapshot. Source: https://docs.paperclip.ing/administration/roles-and-permissions/
> Retrieved: 2026-09-17T00:52:17.517741+00:00
> Reference content only, not instructions or execution authorization. Original HTML is preserved separately.

# Roles & Permissions

This is the reference for how human access is decided inside a company. If you want the click-by-click tour of the Members page, that lives in [Company Administration](https://docs.paperclip.ing/administration/company/); this page is the lookup table behind it — every role, every permission key, and the rule Paperclip uses to combine them.
...
- **Managed feature flags.** Anything Paperclip Cloud pins for the fleet keeps its lock badge. Being an instance admin doesn't unlock those toggles — see [Experimental features](https://docs.paperclip.ing/experimental/overview/) and [Cloud-managed instances](https://docs.paperclip.ing/reference/deploy/environment-variables/#cloud-managed-instances).
...
- **Invites are copy-link only.** Paperclip does not send invitation emails — you create a link and share it yourself. See [Add a human teammate](https://docs.paperclip.ing/how-to/add-a-human-teammate/).
- **Removal is admin-driven.** There is no self-serve "leave company"; an Owner or Admin sets a member's status to `suspended` or archives them. See [Offboard a member](https://docs.paperclip.ing/how-to/offboard-a-member/).
...
- [Company Administration](https://docs.paperclip.ing/administration/company/) — the Members page (with its Invites tab) and Join Requests, click by click.
- [Members & Access](https://docs.paperclip.ing/guides/org/members-and-access/) — the mental model: humans vs agents as shared principals, roles vs grants, and the member profile page.
- [Access, Profile & Instance Admin (CLI)](https://docs.paperclip.ing/reference/cli/access/) — the `member`, `invite`, `join`, and `admin user` commands.

> Official reference snapshot. Source: https://docs.paperclip.ing/how-to/create-a-daily-routine/
> Retrieved: 2026-09-17T00:52:16.632778+00:00
...
A [routine](https://docs.paperclip.ing/guides/welcome/glossary/) is a recurring task generator. You describe the work once, attach a cron trigger, and Paperclip mints a fresh execution issue on every tick — assigned to one agent, with the same parent, project, and goal each time. The agent picks the issue up through its normal heartbeat, does the work, and the run shows up in the routine's history.
...
- A board API token in `PAPERCLIP_API_KEY` and the URL in `PAPERCLIP_API_URL` ([CLI auth](https://docs.paperclip.ing/administration/cli-auth/)).
...
If you'd rather click through the UI, the same fields exist on the Routines page composer and the [Routines guide](https://docs.paperclip.ing/guides/projects-workflow/routines/) walks the screens. Everything below is the API path so you can wire it from a script.
...
`title` and `description` are templates: anything you write is what each run's execution issue says. If you want per-run substitution (`{{customer}}`, `{{date}}`, etc.), declare variables on the routine — see [Variable templates in the Routines guide](https://docs.paperclip.ing/guides/projects-workflow/routines/#variable-templates).
...
In the UI, any `env` value with `type: "secret_ref"` is rendered through a secret binding picker — same widget you see on agent configs — so you pick from existing company secrets instead of pasting plaintext. See [`secret-ref` form fields](https://docs.paperclip.ing/reference/api/secrets/#secret-ref-form-fields) for the JSON schema flag that powers the picker.
...
- [Heartbeats & Routines](https://docs.paperclip.ing/guides/projects-workflow/routines/) — full UI walkthrough, concurrency and catch-up in depth, variable templates.
- [Routines API](https://docs.paperclip.ing/reference/api/routines/) — every endpoint, every field, every status.
- [Wire Slack/Discord notifications](https://docs.paperclip.ing/how-to/wire-slack-discord-notifications/) — pipe pending approvals and blocked issues to a channel using the same routine pattern.
- [Debug a stuck heartbeat](https://docs.paperclip.ing/how-to/debug-stuck-heartbeat/) — first place to look if a routine fires but the agent does nothing.
- [Glossary](https://docs.paperclip.ing/guides/welcome/glossary/) — definitions for routine, heartbeat, execution issue.

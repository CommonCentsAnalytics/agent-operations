> Official reference snapshot. Source: https://docs.paperclip.ing/how-to/back-up-and-restore-a-company/
> Retrieved: 2026-09-17T00:52:17.065292+00:00
...
The CLI shortcut (`paperclipai company export | import`) is fine for one-off work — see [Export & Import](https://docs.paperclip.ing/guides/power/export-import/). The HTTP routes below are what you'll wire into a routine, a CI job, or any agent that needs durable backups.
...
`expectedFileCount` is optional but worth sending: it is your assertion of how many files you put in the body, and the server refuses the import if fewer arrive. See [The two integrity guards](https://docs.paperclip.ing/how-to/back-up-and-restore-a-company/#the-two-integrity-guards) below.
...
- [Export & Import](https://docs.paperclip.ing/guides/power/export-import/) — CLI walkthrough and package format.
- [Heartbeats & Routines](https://docs.paperclip.ing/guides/projects-workflow/routines/) — schedule, concurrency, and catch-up policies for the nightly recipe.
- [Routines API](https://docs.paperclip.ing/reference/api/routines/) — every endpoint for creating and managing routines.
- [Companies API](https://docs.paperclip.ing/reference/api/companies/#export-and-import) — full route table including the board-level paths.
- [Deploy to a VPS or Fly.io](https://docs.paperclip.ing/how-to/deploy-to-vps-or-fly/) — pair with a Postgres backup and an off-host bundle store.

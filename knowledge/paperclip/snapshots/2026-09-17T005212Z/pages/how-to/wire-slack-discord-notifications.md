> Official reference snapshot. Source: https://docs.paperclip.ing/how-to/wire-slack-discord-notifications/
> Retrieved: 2026-09-17T00:52:17.126063+00:00
...
Paperclip does not push outbound webhooks today — the routine + agent pair *is* the push. That keeps the moving parts in one place: you can read the notifier's run history, replay a missed event by re-running the routine, and rotate the channel webhook by updating one secret. See [Heartbeats & Routines](https://docs.paperclip.ing/guides/projects-workflow/routines/) for the underlying model.
...
The URL is itself a bearer token — anyone who has it can post to the channel. Never commit it. Store it as a Paperclip secret on the notifier agent's environment (see [Signing + secrets](https://docs.paperclip.ing/how-to/wire-slack-discord-notifications/#signing--secrets) below).
...
1. **Never commit the URL.** Store it on the notifier agent's environment — for `claude_local`, that's a per-agent env var in the agent config; for `http_webhook`, set it on the receiving service. See [environment variables](https://docs.paperclip.ing/reference/deploy/environment-variables/).
...
If you also want to *receive* webhooks into Paperclip (Stripe → Paperclip routine, GitHub → Paperclip routine), that's the other direction and uses Paperclip's signed-trigger model with `bearer`, `hmac_sha256`, or `github_hmac` modes. Documented in [Routines API → Webhook triggers](https://docs.paperclip.ing/reference/api/routines/#webhook).
...
- [Approvals (board guide)](https://docs.paperclip.ing/guides/day-to-day/approvals/) — what each approval type looks like in the UI.
- [Heartbeats & Routines](https://docs.paperclip.ing/guides/projects-workflow/routines/) — concurrency and catch-up policies in depth.
- [Approvals API](https://docs.paperclip.ing/reference/api/approvals/) — the endpoints the notifier reads.
- [Routines API](https://docs.paperclip.ing/reference/api/routines/) — schedule cron and webhook trigger reference.
- [Debug a stuck heartbeat](https://docs.paperclip.ing/how-to/debug-stuck-heartbeat/) — first place to look if the notifier stops firing.

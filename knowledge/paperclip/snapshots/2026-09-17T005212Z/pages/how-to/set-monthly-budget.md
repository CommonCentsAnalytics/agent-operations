> Official reference snapshot. Source: https://docs.paperclip.ing/how-to/set-monthly-budget/
> Retrieved: 2026-09-17T00:52:16.219609+00:00
...
[Image reference: Budgets tab showing the control plane and active incidents grid](https://docs.paperclip.ing/how-to/set-monthly-budget/user-guides/screenshots/dark/costs/budgets.png)

Paperclip does not push outbound webhooks today. If you want Slack or Discord pings on incidents, run a routine that diffs `GET /api/companies/{COMPANY_ID}/budgets/overview` against a cursor and posts to a webhook — the same shape as the approval notifier. See [Wire Slack/Discord notifications](https://docs.paperclip.ing/how-to/wire-slack-discord-notifications/).
...
[Image reference: Per-agent budget bar at 100% — red, agent shown as paused](https://docs.paperclip.ing/how-to/set-monthly-budget/user-guides/screenshots/dark/costs/agent-budget-100pct-paused.png)
...
- [Costs guide](https://docs.paperclip.ing/guides/day-to-day/costs/) — the full UI walkthrough for the Costs page (Overview, Budgets, Providers, Billers, Finance).
- [Costs API reference](https://docs.paperclip.ing/reference/api/costs/) — every endpoint touched here, plus reading routes (`/costs/summary`, `/costs/by-agent`, `/costs/window-spend`).
- [Handle board approvals for hires](https://docs.paperclip.ing/how-to/handle-board-approvals-for-hires/) — the same governance flow but for new agent budgets at hire time.
- [Require board approval before an agent spends money](https://docs.paperclip.ing/how-to/require-board-approval-before-spend/) — the proactive half of spend control: the agent stops and asks the board before committing to a discrete spend, so the cap here is the safety net rather than the only line of defence.
- [Wire Slack/Discord notifications](https://docs.paperclip.ing/how-to/wire-slack-discord-notifications/) — pipe budget incidents into a channel so you see them before checking the UI.

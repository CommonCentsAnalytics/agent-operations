> Official reference snapshot. Source: https://docs.paperclip.ing/how-to/bring-your-own-agent/
> Retrieved: 2026-09-17T00:52:16.387468+00:00
...
Paperclip is a control plane. The thing that *runs* an agent — Claude Code, an HTTP service you operate, a Python script polling the API — is decoupled from it by an [adapter](https://docs.paperclip.ing/reference/adapters/overview/). This guide shows three ways to wire a non-built-in agent into a Paperclip company, side by side, so you can pick the path that fits your runtime.
...
A Paperclip agent is a database row plus an `adapterType` that tells Paperclip how to wake it. When an issue is assigned and a heartbeat fires, Paperclip resolves the adapter, hands it the wake context, and lets the adapter take over: launch a CLI, post to a webhook, broker a WebSocket, whatever the adapter does. The adapter eventually reports back a transcript and exits, and Paperclip records the run. The full menu of adapters lives in [Adapters Overview](https://docs.paperclip.ing/reference/adapters/overview/); this how-to picks the three that don't require Paperclip to launch a local CLI process.
...
If your external runtime is Hermes Agent, the Hermes-specific walkthrough is [Hermes Gateway](https://docs.paperclip.ing/reference/adapters/hermes-gateway/). It covers the same invite flow plus the URL mapping between Paperclip and the Hermes gateway.
...
Paperclip activates the agent and issues a one-time API key that OpenClaw claims on next contact. Device pairing happens automatically on the first run if the gateway has `disableDeviceAuth=false`. See [OpenClaw Gateway → Onboarding Checklist](https://docs.paperclip.ing/reference/adapters/openclaw-gateway/#onboarding-checklist) for the preflight.
...
For the full hire-approval workflow including denial paths and the OpenClaw skill-sync caveat, see [Handle board approvals for hires](https://docs.paperclip.ing/how-to/handle-board-approvals-for-hires/#5-openclaw-variant-the-invite-prompt-flow).
...
Set `PAPERCLIP_WEBHOOK_SECRET=$SECRET` on your service so the same value is on both ends, and rotate it the same way you'd rotate any production credential. The full field list and request-body shape are in the [HTTP adapter](https://docs.paperclip.ing/reference/adapters/http/) reference.
...
The five steps in the loop — identity, inbox, checkout, work, update — are the same five steps every Paperclip adapter walks through internally. Your script is just the visible version of that contract. The full endpoint list is in the [API reference](https://docs.paperclip.ing/reference/api/overview/).
...
- **Adapter-managed budget tracking.** Run cost telemetry isn't recorded automatically — the [budget enforcement](https://docs.paperclip.ing/how-to/set-monthly-budget/) policy still gates spend, but your script has to report cost manually if you want it tracked.
...
- [Adapters Overview](https://docs.paperclip.ing/reference/adapters/overview/) — every adapter type and when to use it.
- [HTTP adapter](https://docs.paperclip.ing/reference/adapters/http/) — full field list, payload shape, and `Test Environment` behaviour for Option B.
- [OpenClaw Gateway](https://docs.paperclip.ing/reference/adapters/openclaw-gateway/) — transport, device auth, and onboarding checklist for Option A.
- [Connect an agent to a GitHub repo](https://docs.paperclip.ing/how-to/connect-agent-to-github/) — pair this with Option A or B for a coding agent that opens PRs.
- [Handle board approvals for hires](https://docs.paperclip.ing/how-to/handle-board-approvals-for-hires/) — the approval flow each invite/hire passes through.
- [Debug a stuck heartbeat](https://docs.paperclip.ing/how-to/debug-stuck-heartbeat/) — first stop when the wake fires but nothing happens.

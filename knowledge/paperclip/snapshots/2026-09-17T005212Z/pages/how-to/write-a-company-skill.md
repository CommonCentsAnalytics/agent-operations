> Official reference snapshot. Source: https://docs.paperclip.ing/how-to/write-a-company-skill/
> Retrieved: 2026-09-17T00:52:16.625350+00:00
...
A skill is the right tool when you want a *procedure* — a code review checklist, a release-note format, a customer response template — to be available to multiple agents without baking it into every agent's main instructions. It is **not** a place to ship code (that's a [plugin](https://docs.paperclip.ing/administration/plugins/)), and it is **not** a runtime bridge (that's an [adapter](https://docs.paperclip.ing/reference/adapters/overview/)). If a smart human could follow the file as written and produce the right output, it's a skill.
...
- Paperclip running locally or on a server you control. See [Installation](https://docs.paperclip.ing/guides/getting-started/installation/).
- An agent already hired that you want the skill to attach to. See [Hire Your First Agent](https://docs.paperclip.ing/guides/getting-started/your-first-agent/).
...
> **Local paths are live.** Edits to the file on disk show up the next time the skill is read; you don't have to re-import. Useful while you're tuning the description. Switch to a GitHub source (below) before promoting the skill to a real agent — see [Versioning](https://docs.paperclip.ing/how-to/write-a-company-skill/#7-versioning-and-updates).
...
The full list of accepted source forms (npx commands, gist URLs, raw URLs, `owner/repo/skill` shorthand) is in [Company Skills → Source types](https://docs.paperclip.ing/reference/skills/#import-accepted-sources).
...
[Image reference: Skills page showing available skills with their names and descriptions](https://docs.paperclip.ing/how-to/write-a-company-skill/user-guides/screenshots/dark/org/skills-list.png)
...
The response (an `AgentSkillSnapshot`) lists every entry the agent currently has, with `state` (`configured`, `installed`, `available`, etc.) and `mode` (the runtime sync strategy — see below). The full schema is at [Skills reference → Assigning skills to agents](https://docs.paperclip.ing/reference/skills/#3-assigning-skills-to-agents).
...
| `unsupported` | Paperclip records the assignment but cannot push files into the runtime. `openclaw_gateway` falls here — manage skills inside the remote runtime instead. See [Bring your own agent](https://docs.paperclip.ing/how-to/bring-your-own-agent/). |
...
If neither is true, walk down the troubleshooting checklist in the [Skills reference](https://docs.paperclip.ing/reference/skills/#8-troubleshooting-why-a-skill-isnt-loading). The two most common failures: the routing description is too vague for the agent to match, or the adapter's sync mode is `unsupported`.
...
The full set of rules — required vs. optional, bundled-required, materialisation strategy, conflict resolution — is at [Skills reference → Scoping rules](https://docs.paperclip.ing/reference/skills/#4-scoping-rules).
...
**Frontmatter changes don't take effect.** Paperclip's YAML reader is intentionally narrow. Use spaces (not tabs), `- ` for list items, and plain one-line string values for `name` and `description`. Avoid block scalars (`>` and `|`) because they are not folded by the current parser. If a value still parses incorrectly after you simplify it to the supported subset, file a bug; the in-house parser is documented at [Skills reference → Frontmatter fields](https://docs.paperclip.ing/reference/skills/#frontmatter-fields).

**`references/example.md` is missing from the imported file inventory.** Re-import from the parent folder that contains the skill directory (`/Users/me/skills` in this how-to), not the `release-note-writer` folder itself. The parent-folder import walks `release-note-writer/SKILL.md` and the adjacent `references/`, `scripts/`, and `assets/` folders together.

**Bundled skill keeps re-appearing after I delete it.** That's by design — `paperclip` and the other `paperclipai/paperclip/*` skills are re-imported on every list call. To suppress one, run a forked Paperclip build that doesn't ship it.

For deeper debugging — wrong tool list, sync mode confusion, GitHub pin stuck on an old commit — walk the [full troubleshooting list](https://docs.paperclip.ing/reference/skills/#8-troubleshooting-why-a-skill-isnt-loading).

---

## See also

- [Skills guide](https://docs.paperclip.ing/guides/org/skills/) — UI walkthrough, file inventory, write-a-good-skill checklist, trust levels.
- [Skills reference](https://docs.paperclip.ing/reference/skills/) — file shape, install pipeline, canonical keys, scoping, versioning.
- [Agents API → Skills](https://docs.paperclip.ing/reference/api/agents/#skills) — request/response shapes for `GET` and `POST /agents/{id}/skills/sync`.
- [Bring your own agent](https://docs.paperclip.ing/how-to/bring-your-own-agent/) — the OpenClaw / HTTP webhook / custom script paths, including the `unsupported` skill-sync caveat.
- [Handle board approvals for hires](https://docs.paperclip.ing/how-to/handle-board-approvals-for-hires/) — when `desiredSkills` lands in an approval workflow rather than direct create.
- [Add an MCP server to an agent](https://docs.paperclip.ing/how-to/add-mcp-server-to-agent/) — the runtime alternative for *executable* tooling that doesn't fit a skill.

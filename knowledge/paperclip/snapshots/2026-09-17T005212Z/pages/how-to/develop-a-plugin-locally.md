> Official reference snapshot. Source: https://docs.paperclip.ing/how-to/develop-a-plugin-locally/
> Retrieved: 2026-09-17T00:52:17.293616+00:00
...
> **For everyone, not just developers.** If you want to *use* a plugin somebody else has shipped, see [Administration → Plugins](https://docs.paperclip.ing/administration/plugins/) instead. This page is for the moment you want to write or fork one.
...
- The Paperclip CLI on your `PATH`. See [Installation](https://docs.paperclip.ing/guides/getting-started/installation/) if you don't have it yet.
...
- [Administration → Plugins](https://docs.paperclip.ing/administration/plugins/) — operating plugins day-to-day (install, enable/disable, upgrades, permissions).
- [Plugin SDK](https://docs.paperclip.ing/reference/plugins/sdk/) — the worker-side authoring surface: `definePlugin`, `runWorker`, the `PluginContext` clients, manifest types.
- [Write a company skill](https://docs.paperclip.ing/how-to/write-a-company-skill/) — when an agent just needs instructions, not server code.
- [Add an MCP server to an agent's toolkit](https://docs.paperclip.ing/how-to/add-mcp-server-to-agent/) — the other common "extend Paperclip" path; pick MCP for agent tools, plugins for server surface.

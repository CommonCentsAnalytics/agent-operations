> Official reference snapshot. Source: https://docs.paperclip.ing/how-to/require-board-approval-before-spend/
> Retrieved: 2026-09-17T00:52:16.330834+00:00
...
> **Already over a hard cap?** That generates a `budget_override_required` approval automatically — see [Set a monthly budget and enforce it](https://docs.paperclip.ing/how-to/set-monthly-budget/) for that flow. The gate this guide describes is the agent-initiated `request_board_approval`, which is a different pattern.
...
[Image reference: Approvals queue with a pending request_board_approval row](https://docs.paperclip.ing/how-to/require-board-approval-before-spend/user-guides/screenshots/dark/approvals/approvals-list.png)
...
[Image reference: Approve, Reject, and Request Revision buttons](https://docs.paperclip.ing/how-to/require-board-approval-before-spend/user-guides/screenshots/dark/approvals/approve-reject-revision-buttons.png)
...
For the full UI walkthrough, including queue filters and the comments thread, see [Approvals (board guide)](https://docs.paperclip.ing/guides/day-to-day/approvals/).
...
For BYO-agent runtimes that don't get Paperclip-driven wakes at all (the polling pattern in [Bring your own agent → Option C](https://docs.paperclip.ing/how-to/bring-your-own-agent/#option-c--custom-script-no-paperclip-adapter)), poll the approval directly: `GET /api/approvals/$APPROVAL_ID` returns the current status. Loop with backoff and bail when `status != "pending"`.
...
**Combine with budget caps, don't replace them.** The approval gate is enforced by the agent's own instructions. A new agent imported from another company, or a model that ignored its instructions, can bypass the gate. The hard cap from [HT1](https://docs.paperclip.ing/how-to/set-monthly-budget/) catches the bypass and pauses the scope. Run both.
...
**Notifications.** The Approvals queue does not push to Slack or Discord on its own. If your board lives in chat and the queue would otherwise sit, [wire a notification routine](https://docs.paperclip.ing/how-to/wire-slack-discord-notifications/) — the same shape as the hire-approval notifier.

---

## See also

- [Set a monthly budget and enforce it](https://docs.paperclip.ing/how-to/set-monthly-budget/) — the reactive half of spend control. Run alongside this gate.
- [Approvals (board guide)](https://docs.paperclip.ing/guides/day-to-day/approvals/) — the full UI walkthrough for the queue and decision buttons.
- [Approvals API](https://docs.paperclip.ing/reference/api/approvals/) — every endpoint touched here, plus the lifecycle diagram and resubmit semantics.
- [Handle board approvals for hires](https://docs.paperclip.ing/how-to/handle-board-approvals-for-hires/) — the same governance flow for the `hire_agent` approval type.
- [Wire Slack/Discord notifications](https://docs.paperclip.ing/how-to/wire-slack-discord-notifications/) — pipe approvals into a channel so the queue doesn't sit.
- [Debug a stuck heartbeat](https://docs.paperclip.ing/how-to/debug-stuck-heartbeat/) — first stop when the wake fires but the agent doesn't pick the approval up.

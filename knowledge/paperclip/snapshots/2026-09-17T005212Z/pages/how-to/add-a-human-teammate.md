> Official reference snapshot. Source: https://docs.paperclip.ing/how-to/add-a-human-teammate/
> Retrieved: 2026-09-17T00:52:16.195531+00:00
...
**Before you start:** creating invites needs the `users:invite` permission, and approving the person who shows up needs `joins:approve`. Owners and Admins have both by default. If you're not sure what you hold, see [Roles & Permissions](https://docs.paperclip.ing/administration/roles-and-permissions/).
...
You can always change this after they're in (step 5), so don't overthink it. The full breakdown is in [Roles & Permissions](https://docs.paperclip.ing/administration/roles-and-permissions/).

---

## 2. Create the invite link

[Image reference: Invites tab of the Members page](https://docs.paperclip.ing/how-to/add-a-human-teammate/user-guides/screenshots/dark/company/invites.png)
...
The **Open invite** button next to the link lets you preview the join page in another tab. The link is **single-use** and expires after 72 hours — if it goes stale, just create another. For the full tour, see [Company Administration → Invites](https://docs.paperclip.ing/administration/company/#invites).
...
> **If they see "Email and password sign up is not enabled":** your instance has self-service sign-up turned off, so the invitee can't create the account they need to accept the invite. This is controlled by the `auth.disableSignUp` config field (or the `PAPERCLIP_AUTH_DISABLE_SIGN_UP` environment variable). To let invited teammates register, make sure sign-up is enabled — set `auth.disableSignUp` to `false` (its default) or confirm `PAPERCLIP_AUTH_DISABLE_SIGN_UP` isn't set to `true` — then restart the instance and have them reopen the invite link. See [Enable multi-user login](https://docs.paperclip.ing/how-to/enable-multi-user-login/) for the full authenticated-mode setup.
...
[Image reference: Join request queue](https://docs.paperclip.ing/how-to/add-a-human-teammate/user-guides/screenshots/dark/company/join-requests.png)

Click **Approve human** and the person becomes an **active** member with the invite's default role. To adjust their role or hand out extra permissions, stay on **Settings → Members**, click **Edit** on their row, and set the role, status, and any explicit grants in the grants grid, then save. (Explicit grants stick even if you later change their role — see [Roles & Permissions](https://docs.paperclip.ing/administration/roles-and-permissions/#how-grants-combine-precedence).)

---

## 6. Verify

They now appear in **Settings → Members** with an `active` status badge and the role you gave them. That's it — they can sign in and see the company. Every step above (invite created, join requested, approved, membership activated) is written to the [activity log](https://docs.paperclip.ing/guides/day-to-day/activity-log/), so the whole onboarding is auditable after the fact.

---

## Prefer the terminal? CLI equivalents

Every step above has a CLI counterpart, handy for scripting onboarding. They map one-to-one onto the same API the UI uses. See the [Access CLI reference](https://docs.paperclip.ing/reference/cli/access/#invites) for exact payloads.
...
- [Company Administration](https://docs.paperclip.ing/administration/company/) — the Members page (with its Invites tab) and Join Requests in full.
- [Roles & Permissions](https://docs.paperclip.ing/administration/roles-and-permissions/) — what each role and grant actually allows.
- [Offboard a member](https://docs.paperclip.ing/how-to/offboard-a-member/) — the reverse direction when someone leaves.
- [Enable multi-user login](https://docs.paperclip.ing/how-to/enable-multi-user-login/) — required first if your instance is still in local trusted mode.
